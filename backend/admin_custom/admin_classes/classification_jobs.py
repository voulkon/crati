...
class ClassificationJobLogInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ('timestamp', 'level', 'message', 'batch_number', 'decisions_in_batch')
...
class ClassificationJobAdmin(admin.ModelAdmin):
    ...
    def actions_column(self, obj):
        ...
        if obj.status in [JobStatus.PENDING.value, JobStatus.RUNNING.value, JobStatus.PAUSED.value]:
        ...
        if obj.status == JobStatus.COMPLETED.value:
            ...
            buttons.append(
                f'<a href="{view_url}" class="button">View Details</a>'
            )
    ...
    def dashboard(self, request):
        ...
        context = {
            **self.admin_site.each_context(request),
            ...
            'classification_rate': round((classified_count / total_decisions * 100), 2) if total_decisions > 0 else 0,
            'direct_assignment_rate': round((direct_assignments / classified_count * 100), 2) if classified_count > 0 else 0,
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['active_jobs'] = ClassificationJob.objects.filter(
            status__in=[JobStatus.PENDING.value, JobStatus.RUNNING.value, JobStatus.PAUSED.value]
        ).order_by('-created_at')
