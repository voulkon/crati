...
class DecisionAdmin(admin.ModelAdmin):
    ...
    def check_pipeline_health(self, request, queryset):
        ...
        if len(decisions) > 50:
            messages.warning(request, f"Health check limited to 50 decisions (selected {len(decisions)})")
        ...
        if summary['errors'] > 0:
            error_decisions = [
                hc.decision.ada for hc in results['health_checks'] 
                if hc.overall_status == HealthStatus.ERROR
            ][:5]  # Show first 5
            messages.error(
                request,
                f"Decisions with errors: {', '.join(error_decisions)}" +
                (f" and {summary['errors'] - 5} more" if summary['errors'] > 5 else "")
            )
    
    check_pipeline_health.short_description = "🔍 Check pipeline health"
    ...
    def fix_common_issues(self, request, queryset):
        ...
        if decision_count > 20:
            messages.warning(request, f"Auto-fix limited to 20 decisions (selected {decision_count})")
    ...
    fix_common_issues.short_description = "🔧 Attempt auto-fix"
