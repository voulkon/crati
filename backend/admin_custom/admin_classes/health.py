...
class DecisionHealthCheckAdmin(admin.ModelAdmin):
    ...
    def refresh_health_checks(self, request, queryset):
        ...
        for health_check in queryset:
            try:
                ...
                messages.error(request, f"Failed to refresh {health_check.decision.ada}: {str(e)}")
    ...
    def export_health_report(self, request, queryset):
        ...
        response = JsonResponse({'health_checks': data}, json_dumps_params={'indent': 2})
        response['Content-Disposition'] = f'attachment; filename="health_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json"'
        return response
