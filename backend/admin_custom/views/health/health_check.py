...
def fix_entity_data(request, pk):
    ...
    if afms_to_fetch:
        ...
        messages.success(
            request, 
            f"✅ Queued {len(afms_to_fetch)} AFMs for GEMI lookup: {', '.join(afms_to_fetch)}. "
    ...
    if afms_failed:
        messages.warning(
            request,
            f"⚠️ {len(afms_failed)} AFMs previously failed: {', '.join(f'{afm} ({err})' for afm, err in afms_failed)}. "
...
def retry_document_extraction(request, pk):
    ...
    try:
        ...
        try:
            ...
            messages.info(request, f"📄 Reset extraction status from {old_status} to PENDING")
            ...
            if decision.document_url:
                ...
                messages.info(request, "📄 Created extraction record for decision with document URL")
            else:
                messages.error(request, "❌ Decision has no document URL to extract")
                return redirect('admin:health_check_detail', pk=pk)
        
        # Trigger extraction task
        from core.tasks.tasks_document_processing import process_single_decision_document
        process_single_decision_document.delay(decision.id)
        
        messages.success(
            request, 
            f"✅ Queued document extraction task for {decision.ada}. "
...
def reindex_opensearch(request, pk):
    ...
    try:
        ...
        try:
            extraction = decision.text_extraction
            has_text = bool(extraction.raw_text and len(extraction.raw_text.strip()) > 10)
        except:
            pass
        ...
        if not has_text:
            messages.warning(
                request,
                f"⚠️ Decision {decision.ada} has no extracted text. "
        ...
        if success:
            messages.success(request, f"✅ Successfully re-indexed {decision.ada} in OpenSearch")
        else:
            messages.warning(request, f"⚠️ Re-indexing completed but may have issues. Check OpenSearch logs.")
    except Exception as e:
        messages.error(request, f"❌ Failed to re-index: {str(e)}")
...
def reextract_entities(request, pk):
    ...
    try:
        ...
        if existing_count > 0:
            ...
            messages.info(request, f"🗑️ Deleted {existing_count} existing entity relationships")
        ...
        if entities:
            messages.success(
                request,
                f"✅ Extracted {len(entities)} entities from {decision.ada}. "
            ...
            if afms_needing_data:
                messages.info(
                    request,
                    f"💡 {len(afms_needing_data)} entities need company data. "
                    f"Use 'Fetch Company Data' button to queue lookups."
                )
        else:
            messages.warning(request, f"⚠️ No AFM entities found in {decision.ada}")
        
    except Exception as e:
        messages.error(request, f"❌ Failed to extract entities: {str(e)}")
    ...
    try:
        ...
        if not decision.organization:
            if hasattr(decision, 'organization_uid'):
                diagnostics.append(f"Missing organization. UID in data: {decision.organization_uid}")
            else:
                diagnostics.append("Missing organization. No UID found in decision data.")
        
        # Check signers
        signer_count = decision.signers.count()
        if signer_count == 0:
            diagnostics.append("No signers linked. Check if signer data exists in extra_field_values_json.")
        
        # Check units
        unit_count = decision.units.count()
        if unit_count == 0:
            diagnostics.append("No units linked. Check if unit data exists in extra_field_values_json.")
        
        if diagnostics:
            messages.warning(
                request,
                f"⚠️ Relation issues found: {' | '.join(diagnostics)}"
            )
            messages.info(
                request,
                "💡 Automatic relation linking requires re-importing the decision with proper linking logic. "
                "This typically happens during initial import. Check import logs for this ADA."
            )
        else:
            messages.success(request, "✅ All relations appear properly linked")
        
    except Exception as e:
        messages.error(request, f"❌ Failed to check relations: {str(e)}")
...
def update_coverage(request, pk):
    ...
    try:
        ...
        if not decision.organization or not decision.issue_date:
            messages.error(
                request,
                "❌ Cannot update coverage: decision missing organization or issue_date"
            )
            return redirect('admin:health_check_detail', pk=pk)
        ...
        if created:
            messages.success(
                request,
                f"✅ Created coverage record for {decision.organization.label} on {decision.issue_date.date()} "
                f"with {actual_count} decisions"
            )
        else:
            messages.success(
                request,
                f"✅ Updated coverage: {old_count} → {actual_count} decisions "
                f"for {decision.organization.label} on {decision.issue_date.date()}"
            )
        
    except Exception as e:
        messages.error(request, f"❌ Failed to update coverage: {str(e)}")
