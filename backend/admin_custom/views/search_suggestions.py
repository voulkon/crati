    ...
    if request.method == 'POST':
        ...
        if suggestion_type and entity_id:
            try:
                ...
                if not order_input:
                    max_order = SearchSuggestion.objects.aggregate(Max('order'))['order__max']
                ...
                messages.success(request, f'Successfully added suggestion for "{entity_name}" with order {order}')
                return redirect('admin:search_suggestion_manager')
...
def move_suggestion_up(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            previous_suggestion = SearchSuggestion.objects.filter(
...
def move_suggestion_down(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            next_suggestion = SearchSuggestion.objects.filter(
