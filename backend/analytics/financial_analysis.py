...
class FinancialAnalyzer:
    def analyze_spending_patterns(self):
        ...
        df = pd.DataFrame(decisions.values(
            'organization__label',
            'amount',
            'issue_date',
            'financial_year',
        ...
        # Monthly aggregation
        monthly_spending = df.groupby(['month', 'organization__label'])['amount'].sum().unstack(fill_value=0)
        
        return monthly_spending
    
    def analyze_kae_distribution(self):
        """Analyze KAE code distribution and amounts."""
        kae_data = DecisionAmountKAE.objects.values(
            'kae',
            'decision__organization__label',
            'decision__financial_year'
        ).annotate(
    ...
    def find_financial_outliers(self, threshold=3):
        ...
        amounts = Decision.objects.filter(
            amount__isnull=False
        ...
        outlier_decisions = Decision.objects.filter(
            amount__isnull=False
        )[outlier_indices]
