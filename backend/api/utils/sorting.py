...
def apply_decision_sorting(queryset, sort_by, amount_field='amount', date_field='issue_date'):
    """
    ...
    if sort_by == 'recent':
        ...
        return queryset.annotate(
            sort_amount=models.Case(
                models.When(**{f'{amount_field}__isnull': False}, then=models.F(amount_field)),
...
def apply_aggregated_amount_sorting(queryset, sort_by, aggregation_annotation='total_amount', date_field='issue_date'):
    """
    ...
    if sort_by == 'recent':
        ...
        return queryset.annotate(
            sort_amount=models.Case(
                models.When(**{f'{aggregation_annotation}__isnull': False}, then=models.F(aggregation_annotation)),
                default=models.Value(-999999999),
                output_field=models.DecimalField(),
            )
        ).order_by('-sort_amount', f'-{date_field}')
