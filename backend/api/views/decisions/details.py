...
def decision_detail(request, decision_id):
    ...
    try:
        decision = Decision.objects.select_related(
            ...
            'signers', 'units', 'kae_amounts', 'attachments'
        ).get(id=decision_id)  # Using integer ID
        ...
        decision_data = {
            ...
            } if decision.organization else None,
            
            'decision_type': {
                'uid': decision.decision_type.uid,
                'label': decision.decision_type.label
            } if decision.decision_type else None,
...
def decision_entities(request, decision_id):
    ...
    totals_qs = (
        ...
        .filter(decision_id=decision_id)
            ...
            grouped[key] = {
                ...
                'companies': list(
                    Company.objects.filter(afm=rel.entity.afm).values(
                        'ar_gemi', 'afm', 'co_name_el', 'co_names_en',
                        'legal_type_name', 'status_name', 'municipality_name',
                        'prefecture_name', 'city', 'street', 'street_number',
                        'zip_code', 'url', 'email', 'is_branch',
                        'incorporation_date', 'last_updated'
                    )
                ),
                'parent_key_paths': [],
                **totals_map.get(key, {'total_amount': 0.0, 'occurrences': 0, 'currency': 'EUR'})
            }
    ...
    try:
        ...
        for company in companies:
            ...
            company_data = {
                ...
                'capital': [{
                    'capital_stock': float(capital.capital_stock) if capital.capital_stock else None,
                    'currency': capital.currency,
                    'ecsokefalaiikes': float(capital.ecsokefalaiikes) if capital.ecsokefalaiikes else None,
                    'eggiitikes': float(capital.eggiitikes) if capital.eggiitikes else None
...
def decision_related(request, decision_id):
    ...
    try:
        decision = Decision.objects.select_related('organization', 'decision_type').get(id=decision_id)
        ...
        results = [{
            ...
            'organization': {
                'uid': rel.organization.uid,
                'label': rel.organization.label
            } if rel.organization else None,
            'decision_type': {
                'uid': rel.decision_type.uid,
                'label': rel.decision_type.label
            } if rel.decision_type else None
