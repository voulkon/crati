...
@swagger_auto_schema(
    ...
    manual_parameters=[
        ...
        openapi.Parameter('provider', openapi.IN_QUERY, description="Filter by extraction provider", type=openapi.TYPE_STRING),
        openapi.Parameter('status', openapi.IN_QUERY, description="Filter by decision status", type=openapi.TYPE_STRING),
        openapi.Parameter('is_scanned', openapi.IN_QUERY, description="Filter by scanned document status", type=openapi.TYPE_BOOLEAN),
        openapi.Parameter('organization', openapi.IN_QUERY, description="Filter by organization", type=openapi.TYPE_STRING),
        openapi.Parameter('decision_type', openapi.IN_QUERY, description="Filter by decision type", type=openapi.TYPE_STRING),
...
def document_search_api(request):
    ...
    for result in search_results['results']:
        ...
        result_data = {
            'id': doc.id,
            'decision_id': doc.decision.id if doc.decision else None,  # Fixed: use .id instead of .uid
            'ada': doc.decision.ada if doc.decision else None,
            'decision_title': doc.decision.subject if doc.decision else None,
            'organization': doc.decision.organization.label if doc.decision and doc.decision.organization else None,
            'organization_id': doc.decision.organization.uid if doc.decision and doc.decision.organization else None,  # Organization has uid
            'decision_type': str(doc.decision.decision_type) if doc.decision and doc.decision.decision_type else None,
            ...
            'extraction_date': doc.extraction_date.isoformat() if doc.extraction_date else None,
            'issue_date': doc.decision.issue_date.isoformat() if doc.decision and doc.decision.issue_date else None,
        ...
        if result.get('opensearch_source'):
            result_data['content_preview'] = result['opensearch_source'].get('content_preview', '')
        else:
            result_data['content_preview'] = doc.raw_text[:200] + '...' if doc.raw_text and len(doc.raw_text) > 200 else doc.raw_text
    ...
    try:
        ...
        for doc in document_qs:
            serialized_results.append({
                'id': doc.id,
                'decision_id': doc.decision.ada if doc.decision else None,  # Use ADA instead of uid
                'decision_subject': doc.decision.subject if doc.decision else None,
                'organization': doc.decision.organization.label if doc.decision and doc.decision.organization else None,
                'organization_id': doc.decision.organization.uid if doc.decision and doc.decision.organization else None,
                'provider': doc.extraction_provider,
                'status': doc.extraction_status,
                'is_scanned': doc.is_scanned_document,
                'processing_time_ms': doc.processing_time_ms,
                'extraction_date': doc.extraction_date.isoformat() if doc.extraction_date else None,
                'raw_text_preview': doc.raw_text[:200] + '...' if doc.raw_text and len(doc.raw_text) > 200 else doc.raw_text,
                'type': 'document'
            })
