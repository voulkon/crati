...
@swagger_auto_schema(
    ...
    request_body=openapi.Schema(
        ...
        required=['query'],
        properties={
            'query': openapi.Schema(type=openapi.TYPE_STRING, description='The search query that was used'),
            'result_type': openapi.Schema(type=openapi.TYPE_STRING, description='Type of result selected (e.g., organization, signer, document)'),
            'result_id': openapi.Schema(type=openapi.TYPE_STRING, description='ID of the selected result'),
            'result_name': openapi.Schema(type=openapi.TYPE_STRING, description='Name/title of the selected result'),
            'result_url': openapi.Schema(type=openapi.TYPE_STRING, description='URL path to the selected result'),
        }
    ),
...
def track_search_selection_api(request):
    ...
    try:
        ...
        return Response({
            'success': False,
            'message': 'Failed to track selection'
        }, status=500)
