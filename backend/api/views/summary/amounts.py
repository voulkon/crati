    ...
    try:
        ...
        if group_by == 'organization':
            ...
            result = base_query.annotate(
                period=time_function('decision__issue_date')
            ...
            # Format the response
            formatted_result = [{
                'period': item['period'].strftime('%Y-%m' if group_by == 'month' else '%Y'),
                'total_amount': float(item['total_amount'] or 0),
                'decision_count': item['decision_count']
            } for item in result]
        else:
            return Response({'error': f'Invalid group_by parameter: {group_by}'}, status=400)
        ...
        return Response({
            ...
            'period': period,
            'group_by': group_by,
            'total_received': sum(item['total_amount'] for item in formatted_result),
            'total_decisions': sum(item['decision_count'] for item in formatted_result),
...
def organization_expenditures_summary(request, organization_uid):
    ...
    try:
        ...
        if not organization:
            return Response({'error': f'No organization found with UID: {organization_uid}'}, status=404)
...
def top_transactions(request):
    ...
    try:
        ...
        top_transactions = base_query.values(
            'decision__id',
            'decision__ada',
            'decision__subject',
