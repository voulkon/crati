...
def debug_environment(request):
    ...
    try:
        ...
        sock.close()
        connectivity[f"{jaeger_host}:{jaeger_port}"] = "connected" if result == 0 else f"failed-{result}"
    except Exception as e:
...
def test_celery_tracing(request):
    ...
    try:
        ...
        delay_seconds = int(request.data.get('delay', 3)) if hasattr(request, 'data') else 3
...
def test_error_tracing(request):
    ...
    try:
        ...
        return JsonResponse({
            ...
            "message": "Check Jaeger for error traces"
        })
    ...
    try:
        ...
        with tracer.start_as_current_span("verbose-test-main") as main_span:
            ...
            with tracer.start_as_current_span("database-operations") as db_span:
                ...
                user_count = CustomUser.objects.count()
                all_users = list(CustomUser.objects.all()[:3])
...
def test_simple_task(request):
    ...
    try:
        ...
        message = request.data.get('message', 'Hello from Django!') if hasattr(request, 'data') else 'Hello from Django!'
