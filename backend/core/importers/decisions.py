...
from core.models.decisions import DecisionAmountKAE, Attachment
...
class DecisionImporter(BaseImporter):
    ...
    def _fetch_missing_entities(self, entity_ids, entity_type):
        ...
        for entity_id in entity_ids:
            try:
                ...
                if not entity_dto:
                    logger.warning(f"Could not fetch {entity_type} data for ID {entity_id}")
                    continue
                
                # Special handling for signers without organization IDs
                if entity_type == 'signer' and (not hasattr(entity_dto, 'organizationId') or not entity_dto.organizationId):
                    logger.warning(f"Signer {entity_id} has no organization ID - attempting to resolve through units")
                    
                    # Try to resolve organization through signer's units
                    org_id, resolution_path = self._resolve_signer_organization(entity_id, fetcher)
                    ...
                    if org_id:
                        ...
                        logger.info(f"Resolved organization {org_id} for signer {entity_id} through units")
                    else:
                        # Use default organization as last resort
                        default_org_uid = self._ensure_default_organization('signer', entity_id)
                        logger.warning(f"Using default organization for signer {entity_id} - couldn't resolve through units")
                        ...
                        if default_org_uid in self.org_cache:
                            ...
                            default_org_exists = Organization.objects.filter(uid=default_org_uid).exists()
                        ...
                        if not default_org_exists:
                            try:
                                ...
                                logger.error(f"Failed to create default organization: {org_err}")
                
                        entity_dto.organizationId = default_org_uid
                
                # Special handling for units without organization IDs
                if entity_type == 'unit' and (not hasattr(entity_dto, 'organizationId') or not entity_dto.organizationId):
                    ...
                    if org_id:
                        ...
                        logger.warning(f"Using default organization for unit {entity_id} - couldn't find through parents")
                ...
                if entity_type in ['signer', 'unit'] and hasattr(entity_dto, 'organizationId'):
                    org_id = entity_dto.organizationId
                    
                    # Skip entities without organization ID (they'll fail validation)
                    if not org_id:
                        logger.warning(f"{entity_type.capitalize()} {entity_id} has no organization ID, skipping")
                        continue
                    ...
                    if not org_exists:
                        logger.info(f"Organization {org_id} not found for {entity_type} {entity_id}. Fetching it now...")
                        ...
                        if org_dto:
                            ...
                            if org_created_count > 0:
                                ...
                                logger.info(f"Successfully imported organization {org_id} for {entity_type} {entity_id}")
                            else:
                            ...
                            logger.warning(f"Could not fetch organization {org_id} from API. Creating placeholder.")
                            try:
                                ...
                                self.org_cache[org_id] = True  # Mark as existing in cache
                                logger.info(f"Created placeholder for organization {org_id}")
                            except Exception as org_err:
                                logger.error(f"Failed to create placeholder for org {org_id}: {org_err}")
    ...
    def _get_related_pks(self, dto: DecisionDTO) -> Tuple[str | None, List[str], List[str], str | None]:
        """
        ...
        if dto.organizationId:
            ...
            if dto.organizationId in self.org_cache:
                ...
                org_exists = Organization.objects.filter(uid=dto.organizationId).exists()
            ...
            if org_exists:
                ...
                fetched_org_uids = self._fetch_missing_entities([dto.organizationId], 'organization')
                if fetched_org_uids:
        ...
        if dto.signerIds:
            ...
            if len(signer_pk_uids) != len(dto.signerIds):
                ...
                fetched_signer_uids = self._fetch_missing_entities(missing_signers, 'signer')
    ...
    def _sync_kae_amounts(self, decision_instance: Decision, dto: DecisionDTO):
        ...
        # ✅ HANDLE BOTH OLD (objects) AND NEW (dicts) FORMATS - FIRST OCCURRENCE
        dto_kae_codes = set()
        for kae in kae_list:
    ...
    def import_many(self, dtos: Iterable[DecisionDTO]) -> int:
        ...
        for dto in dtos:
            ...
            try:
                ...
                org_uid, signer_uids, unit_uids, decision_type_uid = self._get_related_pks(dto)
    ...
    def _link_amounts_to_relationships(self, decision: Decision):
        ...
        amount_fields = DecisionAmountField.objects.filter(decision=decision, associated_relationship__isnull=True)
    ...
    def _trigger_company_data_fetching(self, entities: List['AFMEntity']): # type: ignore
        ...
        from core.tasks import fetch_company_data_for_entities, fetch_company_data_for_single_afm
        ...
        if afms_needing_data:
            logger.info(f"Triggering company data fetch for {len(afms_needing_data)} AFMs")
            # Trigger async task
    ...
    def _ensure_default_organization(self, entity_type, entity_id):
        ...
        if not default_org_exists:
            try:
                ...
                logger.error(f"Failed to create default organization for {entity_type}: {org_err}")
        ...
        for pattern in amount_patterns:
            ...
            for i, amount in enumerate(amount_info['amounts']):
                ...
                amount_field, created = DecisionAmountField.objects.get_or_create(
                    ...
                    defaults={
                        'amount': decimal.Decimal(str(amount)) if amount is not None else None,
                ...
                if not amount_field.associated_relationship:
                    ...
                    for rel in relationships:
                        ...
                        rel_container = rel.parent_key_path.rsplit('.', 1)[0] if '.' in rel.parent_key_path else rel.parent_key_path
                        
                        # Extract container path from amount (e.g., "sponsor[0].expenseAmount" → "sponsor[0]" or use as-is for "sponsor[0]")
                        amount_container = pattern['parent_path'].rsplit('.', 1)[0] if '.' in pattern['parent_path'] else pattern['parent_path']  # Fixed: use 'parent_path'
                        
                        # Match if they're in the same container AND the amount is a specific field (not a container)
                        if rel_container == amount_container and '.' in pattern['parent_path']:
                            matching_rel = rel
                            break
                ...
                if amount_info['related_afms']:
                    ...
                    if rel:
                        rel.amount = decimal.Decimal(str(amount)) if amount is not None else None
