...
class BaseAIJob(ABC):
    ...
    REQUIRED_RESULT_KEYS = ['success', 'input_tokens', 'output_tokens', 'estimated_cost_usd']
        ...
        try:
            ...
            if items:
                ...
                if not isinstance(result['input_tokens'], int):
                    raise JobValidationError("process_item()['input_tokens'] must be int")
                
                logger.info(f"✓ Job {self.JOB_NAME} validation passed")
                return True
        ...
        for idx, item in enumerate(items, 1):
            try:
                ...
                if result['success']:
                    ...
                    logger.warning(f"Failed to estimate item {idx}: {result.get('error')}")
        ...
        try:
            ...
            for idx, item in enumerate(items, 1):
                try:
                    ...
                    item_record = AIJobExecutionItem.objects.create(
                        ...
                        created_analysis=result.get('created_analysis')  # If analysis was created
                    )
                    ...
                    if result['success']:
                        ...
                        if dry_run:
                            ...
                            total_cost += result.get('actual_cost_usd', result['estimated_cost_usd'])
                    
                    logger.info(f"Processed item {idx}/{len(items)}: {item.get('item_identifier')}")
            ...
            execution_time = (datetime.now() - self.execution.started_at).total_seconds()
    ...
    def create_celery_task(cls, job_definition: AIJobDefinition):
        ...
        def execute_job_task(self, provider: str, model: str, dry_run: bool = False, **kwargs):
            """
            ...
            try:
                ...
                return {
                    ...
                    'total_cost_usd': float(execution.actual_cost_usd or execution.estimated_cost_usd or 0),
                    'dry_run': dry_run
                }
...
def load_job_class(job_definition: AIJobDefinition) -> Type[BaseAIJob]:
    ...
    try:
        ...
        raise ImportError(
            f"Could not import {job_definition.algorithm_module}: {e}"
        )
