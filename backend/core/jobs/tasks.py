...
def register_all_job_tasks():
    ...
    for job_def in active_jobs:
        try:
            ...
            task = job_class.create_celery_task(job_def)
            registered.append(job_def.job_name)
            logger.info(f"✓ Registered Celery task: ai_job.{job_def.job_name}")
        except Exception as e:
            logger.error(f"✗ Failed to register {job_def.job_name}: {e}")
...
def validate_all_jobs():
    ...
    for job_def in active_jobs:
        try:
            ...
            results[job_def.job_name] = {
                'valid': True,
                'error': None
            }
            logger.info(f"✓ {job_def.job_name} validation passed")
        except Exception as e:
            results[job_def.job_name] = {
                'valid': False,
                'error': str(e)
            }
            logger.error(f"✗ {job_def.job_name} validation failed: {e}")
