...
from core.utils.discovery_tracking import (
    DiscoverySource,
    analyze_discovery_overlap,
    get_decisions_by_discovery_source
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--org-uid',
            type=str,
            help='Organization UID for by-org analysis'
        )
        parser.add_argument(
            '--export-file',
            type=str,
    ...
    def show_summary(self):
        ...
        tracked = Decision.objects.filter(
            discovery_sources__isnull=False
        ).exclude(discovery_sources=[]).count()
        
        self.stdout.write(f"Decisions with discovery tracking: {tracked:,} ({tracked/total*100:.1f}%)")
        
        # By first discovery source
        self.stdout.write('\n--- By First Discovery Source ---')
        source_counts = Decision.objects.values(
    ...
    def show_gaps(self, limit):
        ...
        only_default = Decision.objects.filter(
        ...
        only_org = Decision.objects.filter(
    ...
    def show_overlap(self, limit):
        ...
        multi_source = Decision.objects.annotate(
            source_count=JSONLength('discovery_sources')
        ).filter(source_count__gt=1).select_related('organization')[:limit]
        
        self.stdout.write(f"Total decisions in multiple sources: {multi_source.count():,}\n")
    ...
    def analyze_by_org(self, org_uid, limit):
        ...
        if not org_uid:
            self.stdout.write(self.style.ERROR('Error: --org-uid required for by-org mode'))
            return
    ...
    def export_analysis(self, export_file):
        ...
        if not export_file:
            self.stdout.write(self.style.ERROR('Error: --export-file required for export mode'))
            return
        ...
        only_default = Decision.objects.filter(
            ...
            source_count=JSONLength('discovery_sources')
        ).filter(source_count=1).values_list('ada', flat=True)
