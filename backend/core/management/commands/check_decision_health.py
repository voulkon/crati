...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--ada',
            type=str,
            help='Check a specific decision by ADA'
        )
        ...
        parser.add_argument(
            '--organization',
            type=str,
            help='Filter by organization UID'
        )
    ...
    def _display_single_result(self, health_check, verbose=False):
        ...
        self.stdout.write(
            color_func(f"Overall Status: {health_check.overall_status}")
        )
        ...
        for component, status in health_check.component_statuses.items():
            ...
            self.stdout.write(f"  {component.replace('_', ' ').title()}: {color_func(status)}")
    ...
    def _display_bulk_results(self, results, options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"  Healthy: {summary['healthy']}")
        )
        self.stdout.write(
            self.style.WARNING(f"  Warnings: {summary['warnings']}")
        )
        self.stdout.write(
        ...
        problematic = [
            hc for hc in results['health_checks'] 
        ...
        if problematic:
            ...
            for health_check in problematic[:20]:  # Show first 20
                status_symbol = "❌" if health_check.overall_status == HealthStatus.ERROR else "⚠️"
