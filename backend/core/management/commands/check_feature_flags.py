...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for flag in flags_to_check:
            ...
            try:
                ...
                if db_flag:
                    ...
                    self.stdout.write(f'  Database:    {self._format_value(db_value)} (type: {db_flag.value_type})')
                else:
            ...
            self.stdout.write(self.style.SUCCESS(f'  → Final:     {self._format_value(final_value)}'))
        ...
        if clerk_key:
            self.stdout.write(self.style.WARNING(
                f'\n⚠️  Clerk is ACTIVE (CLERK_JWT_PUBLIC_KEY is set)'
            ))
            self.stdout.write('   If not using Clerk, remove CLERK_JWT_PUBLIC_KEY from environment\n')
        else:
    ...
    def _format_value(self, value):
        ...
        if isinstance(value, bool):
            return self.style.SUCCESS('✓ TRUE') if value else self.style.ERROR('✗ FALSE')
