...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--ada', 
            type=str, 
            help='Compare extractors on a specific decision by ADA'
        )
        parser.add_argument(
            '--limit',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if include_async:
            self.stdout.write(self.style.WARNING("⚠️  Async mode enabled - this will trigger worker tasks"))
        ...
        if ada:
            ...
            try:
                ...
                results = comparison_service.compare_extractors(decision, include_async=include_async)
    ...
    def _display_results(self, results):
        ...
        for extractor_name, data in results["extractions"].items():
            ...
            type_indicator = "🔄" if extraction_type == "async" else "⚡"
            ...
            if data["success"]:
                ...
                self.stdout.write(
                    self.style.ERROR(f"  {type_indicator} {extractor_name}: FAILED - {data['error']}")
                )
        ...
        if "sync_extractors" in comparison:
            self.stdout.write(f"  Sync extractors: {', '.join(comparison['sync_extractors'])}")
        if "async_extractors" in comparison:
            self.stdout.write(f"  Async extractors: {', '.join(comparison['async_extractors'])}")
                        ...
                        for extractor_name, extractor_data in value.items():
                            ...
                            for k, v in extractor_data.items():
                                if k == "result" and hasattr(v, 'model_dump'):
                                    serializable_result[key][extractor_name][k] = v.model_dump()
                                else:
    ...
    def _display_summary(self, all_results):
        ...
        self.stdout.write(f"Successful comparisons: {len(successful_comparisons)}/{len(all_results)}")
        ...
        for result in successful_comparisons:
            for extractor_name, data in result["extractions"].items():
                if data["success"]:
                    ...
                    extractor_stats[extractor_name]["times"].append(data["processing_time_ms"])
                    extractor_stats[extractor_name]["text_lengths"].append(data["text_length"])
