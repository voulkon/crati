...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not success or not temp_path:
            self.stdout.write(self.style.ERROR(f"Failed to download PDF for {decision.ada}"))
            return
        ...
        try:
            ...
            if output_file:
                self._save_results(decision.ada, {
                    "decision_info": {
                        "ada": decision.ada,
                    ...
                    "pymupdf": {
                        ...
                        "pages": pymupdf_result.pages_data if hasattr(pymupdf_result, 'pages_data') else None
                    },
                    "docling": {
                        "text": docling_result.text,
                        "page_count": docling_result.page_count,
                        "time_ms": docling_time_ms,
                        "pages": docling_result.pages_data if hasattr(docling_result, 'pages_data') else None,
    ...
    def _display_extractor_result(self, name, result, time_ms):
        ...
        if hasattr(result, 'pages_data') and result.pages_data:
            ...
            for i, page in enumerate(result.pages_data[:3]):  # Show first 3 pages
                self.stdout.write(f"  Page {page['page_number']} ({page['character_count']} chars)")
                preview = page['text'][:100].replace('\n', ' ') + "..." if page['text'] else "(empty)"
                self.stdout.write(f"  Preview: {preview}")
            
            if len(result.pages_data) > 3:
                self.stdout.write(f"  ... plus {len(result.pages_data) - 3} more pages/chunks")
    ...
    def _compare_text_content(self, text1, text2):
        ...
        length_diff_percent = int((length_diff / max(length1, length2)) * 100) if max(length1, length2) > 0 else 0
