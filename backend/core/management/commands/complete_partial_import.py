...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Restored from Redis: {restored} ({100*restored/total:.1f}%)')
        self.stdout.write(f'Assigned to pipeline: {assigned} ({100*assigned/total:.1f}%)')
        self.stdout.write(self.style.WARNING(f'\n⚠️  Missing: {missing} decisions ({100*missing/total:.1f}%)'))
        
        if missing == 0:
            self.stdout.write(self.style.SUCCESS('\n✓ All decisions restored! Job may just need status update.'))
            if not dry_run:
                job.status = ImportJobStatus.COMPLETED
                job.completed_at = timezone.now()
                job.save()
                self.stdout.write(self.style.SUCCESS(f'✓ Updated job to COMPLETED'))
            return
        ...
        if job.organization_id:
            decisions_in_db = decisions_in_db.filter(organization_id=job.organization_id)
        if job.unit_id:
        ...
        if unlinked > 0:
            self.stdout.write(self.style.WARNING(
                f'\n⚠️  Found {unlinked} decisions in DB not linked to this job'
            ))
        ...
        if refetch:
            ...
            if dry_run:
                self.stdout.write(self.style.WARNING('DRY RUN - Would fetch from API with params:'))
                import json
            ...
            while page < total_pages:
                ...
                self.stdout.write(f'  Fetched page {page}/{total_pages}: {len(decisions)} decisions')
            
            self.stdout.write(self.style.SUCCESS(f'\n✓ Fetched {len(all_decisions)} decisions total'))
            
            # Now import them
            from core.importers.decisions import DecisionImporter
            from core.utils.discovery_tracking import DiscoverySource, add_discovery_source_to_decision
            ...
            for i, decision_dto in enumerate(all_decisions, 1):
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f'  ✗ Error on decision {i}: {str(e)}'))
            
            self.stdout.write(f'\n=== Summary ===')
            self.stdout.write(self.style.SUCCESS(f'✓ New decisions: {imported}'))
            self.stdout.write(self.style.SUCCESS(f'✓ Updated decisions: {updated}'))
            self.stdout.write(self.style.SUCCESS(f'✓ Pipeline tasks dispatched: {dispatched}'))
            ...
            self.stdout.write(f'\n✓ Updated ImportJob #{job.id}')
        
        else:
            self.stdout.write(f'\n=== Recommendations ===')
            self.stdout.write('1. Use --refetch-from-api to re-fetch missing decisions from Diavgeia')
            self.stdout.write('2. Or accept partial completion and mark job as PARTIALLY_COMPLETED')
