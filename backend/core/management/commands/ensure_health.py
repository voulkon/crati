...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if show_json:
            self.stdout.write(
                self.style.WARNING(f'📄 Will show extra_field_values_json for up to {show_samples} sample decisions')
            )
    ...
    def _show_entity_extraction_samples(self, results, sample_count):
        ...
        self.stdout.write('\n' + '=' * 80)
        self.stdout.write(self.style.WARNING(f'📋 DETAILED SAMPLES ({sample_count} decisions)'))
        self.stdout.write('=' * 80 + '\n')
        
        # Get the actual decisions
        from django.db.models import Q, Exists, OuterRef
        
        decisions_without_entities = Decision.objects.annotate(
            has_entities=Exists(
        ...
        for i, decision in enumerate(decisions_without_entities, 1):
            ...
            self.stdout.write(f'Subject: {decision.subject[:100]}...' if len(decision.subject) > 100 else f'Subject: {decision.subject}')
            ...
            if decision.extra_field_values_json:
                ...
                json_str = json.dumps(decision.extra_field_values_json, indent=2, ensure_ascii=False)
                
                # Truncate if too long
                if len(json_str) > 1000:
                    self.stdout.write(json_str[:1000] + '\n  ... (truncated, total length: {} chars)'.format(len(json_str)))
                else:
    ...
    def _analyze_extractable_content(self, decision):
        ...
        if not efv:
            self.stdout.write('  No extractable content (empty extra_field_values_json)')
            return
        ...
        if entity_extractions:
            self.stdout.write(f'  ✓ Would extract {len(entity_extractions)} entities:')
            for extraction in entity_extractions[:5]:
                afm = extraction['afm']
                role = extraction['role']
                path = extraction['parent_key_path']
                name = extraction.get('name', 'Unknown')
                self.stdout.write(f'    - AFM: {afm} ({name}) - Role: {role} - Path: {path}')
            if len(entity_extractions) > 5:
                self.stdout.write(f'    ... and {len(entity_extractions) - 5} more')
        else:
            self.stdout.write('  ✗ No entities would be extracted (no AFM fields found)')
        
        if amount_extractions:
            total_amounts = sum(len(e['amount_info']['amounts']) for e in amount_extractions)
            self.stdout.write(f'  ✓ Would extract {total_amounts} amounts from {len(amount_extractions)} locations:')
            ...
            if len(amount_extractions) > 5:
                self.stdout.write(f'    ... and {len(amount_extractions) - 5} more locations')
        else:
            self.stdout.write('  ✗ No amounts would be extracted (no amount fields found)')