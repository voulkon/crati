...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"🔍 Exploring AFM patterns in ALL {total_decisions:,} decisions...")
            self.stdout.write(f"📁 Using temporary database: {self.temp_db_path}")
            
            if self.debug_afm:
                self.stdout.write("🐛 Debug mode enabled - will show first few AFM detections")
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ AFM pattern exploration complete! Results in: {self.output_dir}"))
    ...
    def create_temp_database(self) -> str:
        ...
        cursor.execute('CREATE INDEX idx_inner_key_parent ON inner_key_patterns(parent_key)')
        cursor.execute('CREATE INDEX idx_afm_values_parent ON afm_values(parent_key)')
        cursor.execute('CREATE INDEX idx_examples_parent ON structure_examples(parent_key)')
    ...
    def process_all_decisions(self, total_decisions: int) -> int:
        ...
        self.stdout.write("🚀 Starting AFM pattern analysis with database storage...")
        
        for i in range(0, total_decisions, batch_size):
            batch = list(queryset[i:i + batch_size])
            
            for decision in batch:
                decision_afm_count = self.analyze_decision_afm_patterns(decision)
                processed_count += 1
    ...
    def analyze_decision_afm_patterns(self, decision: Decision) -> int:
        ...
        if self.debug_afm and self.debug_count < 5:
            ...
            self.stdout.write(f"   extra_field_values_json type: {type(decision.extra_field_values_json)}")
        ...
        if self.debug_afm and afm_patterns and self.debug_count < 10:
            self.stdout.write(f"🎯 Found {len(afm_patterns)} AFM patterns in decision {decision.ada}")
    ...
    def find_afm_patterns_in_data(self, data: Any, decision_ada: str, parent_path: str = "") -> List[Dict]:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                patterns.extend(self.find_afm_patterns_in_data(value, decision_ada, new_path))
                
        elif isinstance(data, list):
            # Recurse into list items
            for i, item in enumerate(data):
                new_path = f"{parent_path}[{i}]" if parent_path else f"[{i}]"
                patterns.extend(self.find_afm_patterns_in_data(item, decision_ada, new_path))
            ...
            if any(afm_term in key_lower for afm_term in afm_keywords):
                ...
                if self.is_valid_afm_format(value):
                    ...
                    if self.debug_afm and len(afm_indicators) <= 3:
                        self.stdout.write(f"🎯 AFM DETECTED: field='{key}', value='{value}', cleaned='{cleaned_afm}'")
    ...
    def clean_afm_value(self, value: Any) -> str:
        ...
        cleaned = afm_str.replace('EL', '').replace('GR', '').replace('-', '').replace(' ', '').replace('.', '')
        ...
        try:
            ...
            for parent_key, examples in self.batch_structure_examples.items():
                for example in examples:
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?)
                    ''', (parent_key, example['decision_ada'], 
            ...
            for decision_ada, stats in self.batch_decision_stats.items():
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?)
                ''', (decision_ada, stats['afm_count'], json.dumps(stats['parent_keys'])))
            
            conn.commit()
            self.stdout.write(f"💾 Flushed {self.patterns_since_flush} patterns to temp database")
    ...
    def generate_comprehensive_reports_from_db(self, processed_count: int):
        ...
        self.stdout.write("📈 Generating comprehensive reports from database...")
    ...
    def generate_executive_summary_from_db(self, conn: sqlite3.Connection, processed_count: int):
        ...
        cursor.execute('SELECT COUNT(*) FROM (SELECT DISTINCT parent_key, afm_value FROM afm_values)')
        ...
        # Get top insights
        cursor.execute('SELECT parent_key, total_count FROM parent_key_stats ORDER BY total_count DESC LIMIT 1')
        top_parent = cursor.fetchone()
        
        cursor.execute('SELECT field_name, count FROM afm_field_names ORDER BY count DESC LIMIT 1')
        top_field = cursor.fetchone()
        
        cursor.execute('SELECT length, count FROM afm_length_distribution ORDER BY count DESC LIMIT 1')
        top_length = cursor.fetchone()
        
        # Write report
        report_path = self.output_dir / 'EXECUTIVE_SUMMARY.txt'
        afm_coverage = (decisions_with_afms / processed_count) * 100 if processed_count > 0 else 0
        
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("🎯 AFM PATTERN EXPLORATION - EXECUTIVE SUMMARY\n")
            f.write("=" * 60 + "\n\n")
            
            f.write("📊 KEY METRICS:\n")
            f.write("-" * 20 + "\n")
            f.write(f"Total decisions processed: {processed_count:,}\n")
            f.write(f"Decisions containing AFMs: {decisions_with_afms:,} ({afm_coverage:.1f}%)\n")
            ...
            f.write("-" * 20 + "\n")
            if top_parent:
                f.write(f"Most common parent key: '{top_parent[0]}' ({top_parent[1]:,} occurrences)\n")
            if top_field:
                f.write(f"Most common AFM field name: '{top_field[0]}' ({top_field[1]:,} uses)\n")
            if top_length:
                f.write(f"Most common AFM length: {top_length[0]} digits ({top_length[1]:,} AFMs)\n")
    ...
    def generate_parent_key_analysis_from_db(self, conn: sqlite3.Connection):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("📂 PARENT KEY DETAILED ANALYSIS\n")
            f.write("=" * 50 + "\n\n")
            ...
            for parent_key, total_count, unique_afm_count in cursor.fetchall():
                f.write(f"🔑 PARENT KEY: '{parent_key}' ({total_count:,} occurrences)\n")
                f.write("-" * 50 + "\n")
    ...
    def generate_data_quality_report_from_db(self, conn: sqlite3.Connection):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("📋 AFM DATA QUALITY ANALYSIS\n")
            f.write("=" * 40 + "\n\n")
            
            # AFM length distribution
            f.write("📏 AFM LENGTH DISTRIBUTION:\n")
            f.write("-" * 30 + "\n")
            
            cursor.execute('SELECT length, count FROM afm_length_distribution ORDER BY length')
            total_afms = 0
            ...
            for length, count in length_data:
                ...
                quality = "✅ Standard" if length == 9 else ("⚠️ Non-standard" if 8 <= length <= 12 else "❌ Invalid")
                f.write(f"{length:2d} digits: {count:>8,} AFMs ({percentage:>5.1f}%) {quality}\n")
            ...
            for parent_key, count, unique_afms in cursor.fetchall():
                ...
                f.write(f"{parent_key:<20} | {count:>6,} ({percentage:>5.1f}%) | {unique_afms:>6,} unique AFMs\n")
    ...
    def generate_structure_examples_from_db(self, conn: sqlite3.Connection):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("📋 ENTITY STRUCTURE EXAMPLES\n")
            f.write("=" * 50 + "\n\n")
            
            # Get examples for each parent key
            cursor.execute('SELECT DISTINCT parent_key FROM structure_examples ORDER BY parent_key')
            parent_keys = [row[0] for row in cursor.fetchall()]
            
            for parent_key in parent_keys:
                f.write(f"🔑 PARENT KEY: '{parent_key}'\n")
                f.write("-" * 30 + "\n")
                ...
                for i, (decision_ada, afm_fields_json, raw_data_json) in enumerate(cursor.fetchall(), 1):
    ...
    def generate_extraction_strategy_from_db(self, conn: sqlite3.Connection):
        ...
        cursor.execute('SELECT parent_key, total_count FROM parent_key_stats ORDER BY total_count DESC LIMIT 10')
        top_parents = cursor.fetchall()
        
        cursor.execute('SELECT field_name, count FROM afm_field_names ORDER BY count DESC LIMIT 10')
        top_fields = cursor.fetchall()
        
        with open(guide_path, 'w', encoding='utf-8') as f:
            f.write("🎯 AFM EXTRACTION STRATEGY GUIDE\n")
            f.write("=" * 45 + "\n\n")
    ...
    def generate_json_exports_from_db(self, conn: sqlite3.Connection):
        ...
        cursor.execute('SELECT parent_key, total_count, unique_afm_count FROM parent_key_stats')
        parent_stats = {row[0]: {'count': row[1], 'unique_afms': row[2]} for row in cursor.fetchall()}
