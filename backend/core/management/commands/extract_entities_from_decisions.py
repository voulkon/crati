...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--limit',
            type=int,
            help='Maximum number of decisions to process'
        )
        parser.add_argument(
            '--fetch-companies',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if not options['start_ada']:
            ...
            last_relationship = DecisionEntityRelationship.objects.order_by('-decision__ada').first()
            if last_relationship:
                resume_ada = last_relationship.decision.ada
                self.stdout.write(
                    self.style.WARNING(f"Found previous progress. Resume from ADA: {resume_ada}? (use --start-ada to override)")
                )
        ...
        if processed % 100 == 0:  # Every 100 decisions
            self.stdout.write(f"Progress checkpoint: Last ADA processed: {last_processed_ada}")