...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options.get('check_opensearch'):
            ...
            self.stdout.write(f"Documents added to OpenSearch: {final_count - initial_count}")
            ...
            self.stdout.write(f"Document extractions completed today: {recent_extractions}")
    ...
    def _get_opensearch_count(self, opensearch_service):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f"Could not get OpenSearch count: {e}"))
            return 0
