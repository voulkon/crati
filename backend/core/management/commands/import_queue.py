...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'action',
            type=str,
            choices=['status', 'clear-stale', 'clear-duplicates', 'dispatch-next', 'set-limit'],
    ...
    def show_status(self, queue: ImportJobQueue):
        ...
        if stale_count > 0:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  WARNING: {stale_count} jobs stuck for >6 hours detected!"
                )
            )
            self.stdout.write(
                self.style.WARNING(
                    f"These jobs are blocking the queue. Run:"
                )
            )
            self.stdout.write(
                self.style.WARNING(
        ...
        if status['active_jobs']:
            ...
            for job in status['active_jobs']:
                ...
                # Mark stale jobs
                stale_marker = " ⚠️ STALE!" if age_hours > 6 else ""
    ...
    def clear_duplicates(self, dry_run: bool = False):
        ...
        # Show results
        if duplicates_found:
            self.stdout.write(f"\n📊 Found {len(duplicates_found)} duplicate job(s):\n")
            ...
            if dry_run:
                self.stdout.write(
                    self.style.WARNING(
                        f"\n⚠️  DRY RUN: Would delete {len(duplicates_found)} duplicate jobs"
                    )
                )
                self.stdout.write(
                    self.style.WARNING(
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"\n✓ Deleted {deleted_count} duplicate jobs"
                    )
                )
        else:
