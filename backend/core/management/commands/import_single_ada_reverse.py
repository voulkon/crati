...
from core.models.decisions import Decision, Attachment, DecisionAmountKAE
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"🔄 Analyzing decision rollback for: {ada}")
        ...
        try:
            ...
            self.stdout.write(f"❌ Decision {ada} not found in database")
    ...
    def _analyze_decision(self, ada):
        ...
        try:
            ...
            self.stdout.write(f'✅ Decision found: {decision.ada}')
            ...
            self.stdout.write(f'   Organization: {getattr(decision, "organization_id", "N/A")}')
            ...
            self.stdout.write(f'📊 DecisionEntityRelationship records: {relationships.count()}')
            for rel in relationships:
                self.stdout.write(f'   - ID: {rel.id}, Entity AFM: {rel.entity.afm}, Role: {rel.role}')
            
            # 2. Decision Amount Fields
            amount_fields = DecisionAmountField.objects.filter(decision=decision)
            self.stdout.write(f'💰 DecisionAmountField records: {amount_fields.count()}')
            for amount in amount_fields:
                self.stdout.write(f'   - ID: {amount.id}, Field: {amount.source_field_name}, Amount: {amount.amount}')
            
            # 3. Attachments
            attachments = Attachment.objects.filter(decision=decision)
            self.stdout.write(f'📎 Attachment records: {attachments.count()}')
            for att in attachments:
                self.stdout.write(f'   - ID: {att.id}, Filename: {getattr(att, "filename", "N/A")}')
            
            # 4. KAE Amounts
            kae_amounts = DecisionAmountKAE.objects.filter(decision=decision)
            self.stdout.write(f'💼 DecisionAmountKAE records: {kae_amounts.count()}')
            for kae in kae_amounts:
                self.stdout.write(f'   - ID: {kae.id}, KAE: {kae.kae}, Amount: {kae.amount}')
            
            # 5. AFM Entities analysis
            related_afm_entities = AFMEntity.objects.filter(
                decision_relationships__decision=decision
            ).distinct()
            self.stdout.write(f'🏢 Related AFM entities: {related_afm_entities.count()}')
            
            orphaned_entities = []
            for entity in related_afm_entities:
                other_decisions_count = DecisionEntityRelationship.objects.filter(
                    entity=entity
                ).exclude(decision=decision).count()
                
                self.stdout.write(f'   - AFM: {entity.afm}, Name: {entity.name[:50] if entity.name else "N/A"}...')
                self.stdout.write(f'     Other decisions using this entity: {other_decisions_count}')
                
                if other_decisions_count == 0:
                    orphaned_entities.append(entity)
                    self.stdout.write(f'     ⚠️  This entity will be orphaned after deletion!')
    ...
    def _get_user_confirmation(self, keep_entities):
        ...
        if keep_entities:
            self.stdout.write('🔄 SAFE ROLLBACK - Will delete decision but keep AFM entities')
        else:
            self.stdout.write('🚨 FULL ROLLBACK - Will delete decision and orphaned AFM entities')
    ...
    def _execute_rollback(self, decision, keep_entities):
        ...
        self.stdout.write(f'\n{"🔄 SAFE" if keep_entities else "🚨 FULL"} ROLLBACK - Starting deletion...')
        ...
        try:
            with transaction.atomic():
                ...
                decision.delete()
                self.stdout.write(f'✅ Deleted decision {ada}')
                ...
                if not keep_entities and orphaned_entities:
                    ...
                    for entity in orphaned_entities:
                        ...
                        if not DecisionEntityRelationship.objects.filter(entity=entity).exists():
                            afm = entity.afm
                            entity.delete()
                            orphaned_count += 1
                            self.stdout.write(f'🗑️  Deleted orphaned AFM entity: {afm}')
                    
                    self.stdout.write(f'✅ Cleanup complete: {orphaned_count} AFM entities removed')
                elif keep_entities:
                    self.stdout.write('ℹ️  AFM entities preserved for potential reuse')
                
        except Exception as e:
            self.stdout.write(f'❌ Rollback failed: {str(e)}')
            raise
    ...
    def _verify_rollback(self, ada):
        ...
        try:
            ...
            self.stdout.write(f'⚠️  Decision {ada} still exists in database')
        except Decision.DoesNotExist:
            self.stdout.write(f'✅ Decision {ada} successfully removed')

        # Check DecisionEntityRelationship cleanup
        remaining_relationships = DecisionEntityRelationship.objects.filter(
            decision__ada=ada
        ).count()
        self.stdout.write(f'📊 Remaining DecisionEntityRelationship records: {remaining_relationships}')

        # Check DecisionAmountField cleanup  
        remaining_amounts = DecisionAmountField.objects.filter(
            decision__ada=ada
        ).count()
        self.stdout.write(f'💰 Remaining DecisionAmountField records: {remaining_amounts}')

        if remaining_relationships == 0 and remaining_amounts == 0:
            self.stdout.write('✅ All related data successfully cleaned up')
        else:
            self.stdout.write('⚠️  Some related data may still exist')
        
        self.stdout.write(f'\n🎉 Rollback completed for decision {ada}')