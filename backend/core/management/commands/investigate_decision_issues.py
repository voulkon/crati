...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--component',
            choices=['ingestion', 'relations', 'entities', 'document_extraction', 'opensearch', 'coverage'],
        ...
        parser.add_argument(
            '--export-report',
            type=str,
            help='Export investigation report to file'
        )
    ...
    def handle(self, *args, **options):
        ...
        if not problematic_decisions:
            self.stdout.write(
                self.style.SUCCESS('No problematic decisions found! 🎉')
            )
            return
        ...
        if options['export_report']:
            self._export_investigation_report(investigation_results, options['export_report'])
    ...
    def _investigate_decision(self, health_check, options):
        ...
        for component in ['ingestion', 'relations', 'entities', 'document_extraction', 'opensearch', 'coverage']:
            ...
            if component_status in [HealthStatus.ERROR, HealthStatus.WARNING]:
                issue_details = self._investigate_component(component, decision, health_check, options)
    ...
    def _investigate_ingestion(self, decision, options):
        ...
        for field in required_fields:
            if not getattr(decision, field, None):
                ...
                analysis['fix_suggestions'].append(f"Refetch decision data to populate {field}")
        
        # Check data quality
        if decision.subject and len(decision.subject.strip()) < 5:
            analysis['technical_analysis']['invalid_data'].append('Subject too short')
            analysis['fix_suggestions'].append("Subject appears truncated - refetch from API")
    ...
    def _investigate_relations(self, decision, options):
        ...
        if analysis['technical_analysis']['signer_count'] == 0:
            analysis['fix_suggestions'].append("Check if signers were properly imported")
    ...
    def _investigate_entities(self, decision, options):
        ...
        if entity_rels.exists():
            needing_data = sum(1 for rel in entity_rels if rel.afm_entity and rel.afm_entity.needs_company_data_fetch)
            analysis['technical_analysis']['entities_needing_company_data'] = needing_data
            
            if needing_data > 0:
                analysis['fix_suggestions'].append("Trigger company data fetching for entities")
                analysis['fixable'] = True
        else:
            # Check if extraction was attempted
            if hasattr(decision, 'text_extraction') and decision.text_extraction.extraction_status == ProcessingStatus.COMPLETED:
                analysis['technical_analysis']['extraction_attempted'] = True
                analysis['fix_suggestions'].append("Re-run entity extraction on document text")
                analysis['fixable'] = True
            else:
                analysis['fix_suggestions'].append("Ensure document extraction completes first")
    ...
    def _investigate_document_extraction(self, decision, options):
        ...
        if not analysis['technical_analysis']['has_document_url']:
            analysis['fix_suggestions'].append("No document URL available - extraction not possible")
            return analysis
        
        try:
            extraction = decision.text_extraction
            analysis['technical_analysis']['extraction_exists'] = True
            analysis['technical_analysis']['extraction_status'] = extraction.extraction_status
            ...
            if extraction.extraction_status == ProcessingStatus.FAILED:
                analysis['fix_suggestions'].append("Retry document extraction with different provider")
                analysis['fixable'] = True
            ...
            analysis['fix_suggestions'].append("Create extraction record and queue processing")
            analysis['fixable'] = True
    ...
    def _investigate_opensearch(self, decision, options):
        ...
        if not analysis['technical_analysis']['should_be_indexed']:
            analysis['fix_suggestions'].append("Document extraction needed before indexing")
            return analysis
        
        # Test OpenSearch connectivity
        try:
            analysis['technical_analysis']['opensearch_connectivity'] = self.opensearch_service.health_check()
        except Exception as e:
            analysis['fix_suggestions'].append(f"OpenSearch connectivity issue: {str(e)}")
            return analysis
            ...
            try:
                ...
                if test_query.strip():
                    ...
                    if not analysis['technical_analysis']['search_test_passed']:
                        analysis['fix_suggestions'].append("Document indexed but not searchable - re-index")
                        analysis['fixable'] = True
            except Exception as e:
    ...
    def _investigate_coverage(self, decision, options):
        ...
        if not decision.issue_date or not decision.organization:
            analysis['fix_suggestions'].append("Missing required data (issue_date or organization)")
            return analysis
        ...
        if not coverage:
            analysis['fix_suggestions'].append("Regenerate coverage records for this date/organization")
            analysis['fixable'] = True
        else:
            ...
            if abs(coverage.decision_count - actual_count) > 1:
                ...
                analysis['fix_suggestions'].append(f"Coverage count mismatch: recorded={coverage.decision_count}, actual={actual_count}")
                analysis['fixable'] = True
    ...
    def _fix_document_extraction(self, decision, issue):
        ...
        self.stdout.write(f"    ✓ Queued document processing for {decision.ada}")
    ...
    def _fix_opensearch(self, decision, issue):
        ...
        try:
            ...
            if extraction.extraction_status == ProcessingStatus.COMPLETED:
                ...
                self.stdout.write(f"    ✓ Re-triggered OpenSearch indexing for {decision.ada}")
        except Exception as e:
            self.stdout.write(f"    ✗ Failed to fix OpenSearch: {str(e)}")
    ...
    def _fix_entities(self, decision, issue):
        ...
        if entity_rels.exists():
            ...
            if afms:
                ...
                self.stdout.write(f"    ✓ Triggered company data fetching for {len(afms)} entities")
    ...
    def _fix_coverage(self, decision, issue):
        ...
        try:
            update_organization_coverage(
                sender=type(decision),
                instance=decision
            )
            self.stdout.write(f"    ✓ Updated coverage records for {decision.ada}")
        except Exception as e:
            self.stdout.write(f"    ✗ Failed to fix coverage: {str(e)}")
