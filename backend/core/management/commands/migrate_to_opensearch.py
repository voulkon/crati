...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        base_filter = Q(extraction_status='COMPLETED') & Q(raw_text__isnull=False) & ~Q(raw_text='')
        ...
        queryset = DocumentExtraction.objects.filter(
            base_filter
        ).select_related('decision', 'decision__organization', 'decision__decision_type').order_by('id')
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN - No actual migration will occur"))
            ...
            for i, extraction in enumerate(queryset[:5]):
                ...
                content_preview = extraction.raw_text[:100] if extraction.raw_text else "No content"
                
                self.stdout.write(f"  {i+1}. ID: {extraction.id}")
                self.stdout.write(f"     ADA: {decision.ada}")
                self.stdout.write(f"     Subject: {decision.subject[:50] if decision.subject else 'No subject'}...")
        ...
        for batch_start in range(0, total_count, batch_size):
            ...
            self.stdout.write(f"Processing batch {batch_start//batch_size + 1}/{(total_count-1)//batch_size + 1} (documents {batch_start+1}-{batch_end})")
            ...
            for extraction in batch:
                try:
                    ...
                    document_data = {
                        ...
                        'organization': str(decision.organization) if decision.organization else '',  # Fixed!
                        'decision_type': str(decision.decision_type) if decision.decision_type else '',  # Fixed!
                    ...
                    if success:
                        processed += 1
                        if processed % 50 == 0:
                            self.stdout.write(f"  Processed {processed}/{total_count} documents")
                    else:
                        errors += 1
                    ...
                    errors += 1
                    self.stdout.write(self.style.ERROR(f"  Error processing document ID {extraction.id}: {e}"))
