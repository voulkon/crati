...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if test_mode:
            max_decisions = 10
            self.stdout.write("🧪 TEST MODE: Processing only 10 decisions with detailed output")
        ...
        self.stdout.write(f"🔄 Re-importing {total_decisions:,} decisions in batches of {batch_size}")
        ...
        for i in range(0, total_decisions, batch_size):
            ...
            for decision in batch:
                try:
                    ...
                    if not fresh_dto:
                        self.stdout.write(f"❌ Could not fetch {decision.ada}")
                    ...
                    new_fields_in_this_decision = set(new_extra_fields.keys()) - set(original_extra_fields.keys())
                    if new_fields_in_this_decision:
                        new_fields_found.update(new_fields_in_this_decision)
                        self.stdout.write(f"🎯 NEW FIELDS in {decision.ada}: {new_fields_in_this_decision}")
                        
                        if test_mode:
                            self.stdout.write("📋 Original extra_field_values_json:")
                            self.stdout.write(json.dumps(original_extra_fields, indent=2, ensure_ascii=False))
                            self.stdout.write("📋 New extra_field_values_json:")
                            self.stdout.write(json.dumps(new_extra_fields, indent=2, ensure_ascii=False))
                    ...
                    if processed % 50 == 0:
                        ...
                        self.stdout.write(f"   Updated: {updated_count}, New fields found: {len(new_fields_found)}")
