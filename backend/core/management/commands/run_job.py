...
from core.models.ai_pricing import AIJobDefinition, AIJobExecution
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        # Execution mode
        parser.add_argument('--dry-run', action='store_true', help='Estimate only, do not call AI')
        parser.add_argument('--async', action='store_true', dest='async_mode', help='Run via Celery (background)')
        ...
        # Output
        parser.add_argument('--verbose', action='store_true', help='Show per-item progress')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'{mode_str}: {job_def.display_name}{async_str}'))
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('\n⚠ DRY RUN MODE - No actual AI calls will be made'))
        ...
        if async_mode:
            try:
                ...
                self.stdout.write(self.style.SUCCESS(f'✓ Job queued!'))
        ...
        try:
            ...
            if dry_run:
                ...
                self.stdout.write(f'\nEstimated cost: ${float(execution.estimated_cost_usd):.4f}')
                self.stdout.write(f'Actual cost: ${float(execution.actual_cost_usd or 0):.4f}')
            ...
            if execution.items_failed > 0:
                ...
                for item in failed_items:
                    self.stdout.write(f'  • {item.item_identifier}: {item.error_message}')
            ...
            self.stdout.write(f'  /api/admin/core/aijobexecution/{execution.id}/change/')
            self.stdout.write(f'\n{"="*80}\n')
