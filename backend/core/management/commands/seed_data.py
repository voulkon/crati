...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--dictionaries", 
            action="store_true", 
            help="Load dictionaries"
        )
        parser.add_argument(
            "--dictionary-items", 
            action="store_true", 
            help="Load dictionary items"
        )
        parser.add_argument(
            "--types", 
            action="store_true", 
            help="Load act types and extra fields"
        )
        parser.add_argument(
            "--geodata", 
            action="store_true", 
            help="Load organization geodata"
        )
        parser.add_argument(
            "--decisions", 
            action="store_true", 
            help="Load a sample set of decisions"
        )
        parser.add_argument(
            "--decision-start",
        ...
        parser.add_argument(
            "--all", 
            action="store_true", 
            help="Load all available data types"
        )
        parser.add_argument(
            "--force", 
            action="store_true", 
            help="Force update of existing data"
        )
        parser.add_argument(
            "--lite",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if options["all"]:
            ...
            if options["geodata"]:
                geodata_result = service.seed_organization_geodata(force=options["force"])
            ...
            if options["decisions"]:
                ...
                if not options["types"] and not options["all"]:
                    types_result = service.seed_types(force=options["force"])
                    self.report_single_result("act types (pre-req for decisions)", types_result)
    ...
    def report_single_result(self, entity_type, result):
        if result.get("seeded", False):
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Successfully seeded {count} {entity_type}")
            )
            if "message" in result:
                self.stdout.write(f"  {result['message']}")
        else:
            self.stdout.write(
                self.style.WARNING(f"⚠ Skipped seeding {entity_type}: {result.get('message', 'Already exists')}")
            )
