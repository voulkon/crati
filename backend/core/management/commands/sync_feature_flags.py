...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.MIGRATE_HEADING('Syncing Feature Flags with Environment'))
            ...
            with transaction.atomic():
                for flag in FeatureFlag.objects.filter(is_active=True):
                    try:
                        ...
                        self.stdout.write(
                            self.style.ERROR(f'  ✗ Error with {flag.key}: {str(e)}')
                        )
        ...
        if updated_count > 0:
            self.stdout.write(
                self.style.SUCCESS(f'  Updated: {updated_count} flag(s)')
            )
        
        if no_change_count > 0:
            self.stdout.write(
                f'  No change: {no_change_count} flag(s)'
            )
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'\n✓ Feature flags synced successfully!')
            )
