    ...
    def get_stats(self):
        with self.lock:
            ...
            return {
                ...
                'failed': self.failed,
                'avg_duration': self.total_duration / self.successful if self.successful > 0 else 0,
                'p50': p50,
                'p95': p95,
                'p99': p99,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.SUCCESS("🔍 DATABASE PERFORMANCE DIAGNOSTIC TOOL"))
        self.stdout.write(self.style.SUCCESS("   For Docker vs Local PostgreSQL Comparison"))
        ...
        run_all = options['all'] or not any([
            options['quick'], options['benchmark'], 
            options['config'], options['io'], options['network']
        ])
        ...
        if options['quick']:
            ...
            self.stdout.write("\n" + "="*80)
            self.stdout.write(self.style.HTTP_INFO("📊 CONNECTION POOL LOAD TESTS"))
            self.stdout.write("="*80)
    ...
    def run_network_latency_tests(self):
        ...
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.HTTP_INFO("🌐 NETWORK LATENCY TESTS"))
        self.stdout.write("="*80)
        ...
        if tcp_times:
            ...
            self.stdout.write(f"     Std Dev: {statistics.stdev(tcp_times):.2f}ms" if len(tcp_times) > 1 else "")
            
            if statistics.mean(tcp_times) > 1.0:
                self.stdout.write(self.style.ERROR(
                    f"\n     ⚠️  HIGH LATENCY: Avg {statistics.mean(tcp_times):.2f}ms > 1ms"
                ))
                self.stdout.write(self.style.ERROR(
        ...
        self.stdout.write(f"     P95: {sorted(query_times)[int(len(query_times)*0.95)]:.2f}ms")
        ...
        if tcp_times and query_times:
            ...
            self.stdout.write(f"     Network (TCP connect): {network_overhead:.2f}ms ({network_overhead/total_query*100:.1f}%)")
            self.stdout.write(f"     DB Processing: {db_processing:.2f}ms ({db_processing/total_query*100:.1f}%)")
    ...
    def run_connection_establishment_tests(self):
        ...
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.HTTP_INFO("🔌 CONNECTION ESTABLISHMENT TESTS"))
        self.stdout.write("="*80)
        ...
        if overhead > 10:
            self.stdout.write(self.style.ERROR(
                f"\n     ⚠️  HIGH OVERHEAD: New connections take {overhead:.2f}ms extra"
            ))
            self.stdout.write(self.style.ERROR(
    ...
    def run_postgresql_config_analysis(self):
        ...
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.HTTP_INFO("⚙️  POSTGRESQL CONFIGURATION ANALYSIS"))
        self.stdout.write("="*80)
        ...
        with connection.cursor() as cursor:
            for setting, default, description in settings_to_check:
                try:
                    cursor.execute("SELECT setting, unit FROM pg_settings WHERE name = %s", [setting])
        ...
        with connection.cursor() as cursor:
            ...
            if '/var/lib/postgresql/data' in data_dir:
                self.stdout.write(self.style.WARNING("     📦 Appears to be running in a container"))
            ...
            if fsync == 'off':
                self.stdout.write(self.style.ERROR("     ⚠️  fsync is OFF - data durability at risk!"))
    ...
    def run_query_benchmarks(self):
        ...
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.HTTP_INFO("🚀 QUERY BENCHMARKS"))
        self.stdout.write("="*80)
        ...
        times = self._run_query_benchmark(
            "SELECT COUNT(*) FROM core_decision",
            iterations=20
        )
    ...
    def _print_benchmark_stats(self, times):
        ...
        if not times:
            ...
            return
        self.stdout.write(f"     Min: {min(times):.2f}ms | Max: {max(times):.2f}ms | Avg: {statistics.mean(times):.2f}ms")
        if len(times) > 1:
            self.stdout.write(f"     Std Dev: {statistics.stdev(times):.2f}ms | P95: {sorted(times)[int(len(times)*0.95)]:.2f}ms")
    ...
    def run_io_benchmarks(self):
        ...
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.HTTP_INFO("💾 I/O PERFORMANCE TESTS"))
        self.stdout.write("="*80)
        ...
        with connection.cursor() as cursor:
            ...
            if ratio and ratio < 99:
                self.stdout.write(self.style.WARNING(
                    f"\n     ⚠️  Cache hit ratio {ratio:.2f}% < 99% - consider increasing shared_buffers"
                ))
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write(f"     {'Table':<30} {'Seq Scans':>12} {'Idx Scans':>12} {'Seq/Idx Ratio':>15}")
            self.stdout.write("     " + "-"*70)
            for row in results:
                table, seq_scan, seq_read, idx_scan, idx_fetch = row
                idx_scan = idx_scan or 0
                ratio = f"{seq_scan/idx_scan:.2f}" if idx_scan > 0 else "∞"
                self.stdout.write(f"     {table:<30} {seq_scan:>12,} {idx_scan:>12,} {ratio:>15}")
        ...
        with connection.cursor() as cursor:
            ...
            """)
            results = cursor.fetchall()
            self.stdout.write(f"     {'Table':<25} {'Total':>10} {'Data':>10} {'Index':>10} {'Dead Rows':>12}")
            self.stdout.write("     " + "-"*70)
            for row in results:
                table, total, data, idx, live, dead = row
                dead_pct = f"({dead*100/live:.1f}%)" if live > 0 else ""
                self.stdout.write(f"     {table:<25} {total:>10} {data:>10} {idx:>10} {dead:>8,} {dead_pct}")
    ...
    def run_pg_stat_statements_analysis(self):
        ...
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.HTTP_INFO("📈 PG_STAT_STATEMENTS ANALYSIS"))
        self.stdout.write("="*80)
        ...
        with connection.cursor() as cursor:
            ...
            if not exists:
                self.stdout.write(self.style.WARNING(
                    "\n  ⚠️  pg_stat_statements extension not installed"
                ))
                self.stdout.write("     To enable: CREATE EXTENSION pg_stat_statements;")
                self.stdout.write("     Also add to postgresql.conf: shared_preload_libraries = 'pg_stat_statements'")
                return
            ...
            try:
                ...
                if results:
                    ...
                    for row in results:
                        ...
                        self.stdout.write(f"     {query_short:<50} {calls:>8,} {total:>12,.2f} {avg:>10,.2f}")
                else:
                    self.stdout.write("     No query statistics available yet")
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"     Error querying pg_stat_statements: {e}"))
    ...
    def print_diagnostic_summary(self):
        ...
        self.stdout.write("\n" + "="*80)
        self.stdout.write(self.style.HTTP_INFO("📋 DIAGNOSTIC SUMMARY & RECOMMENDATIONS"))
        self.stdout.write("="*80)
        ...
        if issues:
            for idx, issue in enumerate(issues, 1):
                ...
                style = self.style.ERROR if severity == 'ERROR' else (
                    self.style.WARNING if severity == 'WARNING' else self.style.HTTP_INFO
                )
    ...
    def simulate_request(self, request_num, stats):
    ...
    def run_test(self, test_name, num_requests, concurrent):
        ...
        self.stdout.write(f"  Requests: {num_requests} | Concurrent Workers: {concurrent}\n")
        ...
        for i in range(num_requests):
            ...
            thread = threading.Thread(target=self.simulate_request, args=(i, stats))
            thread.start()
        ...
        self.stdout.write(f"      Avg: {final_stats['avg_duration']*1000:.2f}ms | Min: {final_stats['min']*1000:.2f}ms | Max: {final_stats['max']*1000:.2f}ms")
        self.stdout.write(f"      P50: {final_stats['p50']*1000:.2f}ms | P95: {final_stats['p95']*1000:.2f}ms | P99: {final_stats['p99']*1000:.2f}ms")
        ...
        if final_count > initial_count + 2:  # Allow for 2 extra connections
            self.stdout.write(self.style.ERROR(
                f"\n  ⚠️  WARNING: Connections increased from {initial_count} to {final_count}"
            ))
            self.stdout.write(self.style.ERROR(
        ...
        if peak_count > concurrent + 5:  # Allow some overhead
            self.stdout.write(self.style.WARNING(
                f"\n  ⚠️  NOTICE: Peak connections ({peak_count}) >> concurrent workers ({concurrent})"
            ))
            self.stdout.write(self.style.WARNING(
