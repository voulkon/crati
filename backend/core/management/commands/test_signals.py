...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not organization:
            self.stdout.write(self.style.ERROR('No organizations found. Cannot create test decision.'))
            return
        
        # 4. Find a signer to use
        signer = Signer.objects.first()
        if not signer:
            self.stdout.write(self.style.WARNING('No signers found, testing only organization signals'))
        ...
        try:
            ...
            decision = Decision.objects.get(id=decision_id) if isinstance(decision_id, int) else decision_id
        ...
        if org_coverage:
            self.stdout.write(self.style.SUCCESS(
                f'✅ SUCCESS: Organization coverage created! Count: {org_coverage.decision_count}'
            ))
        else:
            self.stdout.write(self.style.ERROR(
                '❌ FAIL: No organization coverage record was created via signal'
            ))
        ...
        if signer:
            ...
            if signer_coverage:
                self.stdout.write(self.style.SUCCESS(
                    f'✅ SUCCESS: Signer coverage created! Count: {signer_coverage.decision_count}'
                ))
            else:
                self.stdout.write(self.style.ERROR(
                    '❌ FAIL: No signer coverage record was created via signal'
                ))
        ...
        if org_coverage_after is None:
            self.stdout.write(self.style.SUCCESS(
                '✅ SUCCESS: Organization coverage was removed after deletion'
            ))
        elif org_coverage_after.decision_count < org_coverage.decision_count:
            self.stdout.write(self.style.SUCCESS(
                f'✅ SUCCESS: Organization coverage was updated after deletion (count: {org_coverage_after.decision_count})'
            ))
        else:
            self.stdout.write(self.style.ERROR(
                '❌ FAIL: Organization coverage was not updated after deletion'
            ))
