...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'ada',
            type=str,
            help='The ADA of the decision to process'
        )
        parser.add_argument(
            '--force',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"🔧 Testing Pipeline for Decision: {ada}"))
        ...
        if use_async:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"✅ Queued pipeline task for {ada}\n"
                f"   Task ID: {result.id}\n"
    ...
    def _print_health_check(self, health_check: DecisionHealthCheck):
        ...
        self.stdout.write(f"\n📊 Overall Status: {status_icon.get(health_check.overall_status, '?')} {health_check.overall_status}")
        self.stdout.write("\nComponent Status:")
        self.stdout.write("-" * 80)
        
        components = [
            ('Ingestion', health_check.ingestion_status, health_check.ingestion_error_message),
            ('Entity Extraction', health_check.entities_status, health_check.entities_error_message),
            ('Company Enrichment', health_check.relations_status, health_check.relations_error_message),
            ('Document Processing', health_check.document_extraction_status, health_check.document_extraction_error_message),
            ('OpenSearch Indexing', health_check.opensearch_status, health_check.opensearch_error_message),
            ('Coverage Metrics', health_check.coverage_status, health_check.coverage_error_message),
        ]
