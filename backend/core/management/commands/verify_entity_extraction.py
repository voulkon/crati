...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        decisions_with_efv = Decision.objects.filter(
        ...
        decisions_with_entities = DecisionEntityRelationship.objects.values('decision').distinct().count()
        ...
        entities_needing_lookup = AFMEntity.objects.filter(
            Q(gemi_lookup_attempted__isnull=True) | Q(gemi_lookup_success=False)
        ).exclude(
            decision_relationships__role='organization'
        ).count()
...
Companies in database: {total_companies:,}
        ...
        if decisions_with_efv > decisions_with_entities:
            unprocessed = Decision.objects.filter(
                ...
                ada__in=DecisionEntityRelationship.objects.values_list('decision__ada', flat=True)
            )[:5]
