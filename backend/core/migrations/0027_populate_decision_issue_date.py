...
def populate_decision_issue_date_forward(apps, schema_editor):
    ...
    updated_count = 0
    for health_check in DecisionHealthCheck.objects.select_related('decision').iterator():
        if health_check.decision and health_check.decision.issue_date and not health_check.decision_issue_date:
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            populate_decision_issue_date_forward,
            populate_decision_issue_date_reverse
        ),
    ]