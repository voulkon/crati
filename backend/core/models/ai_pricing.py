...
class AIModelPricing(models.Model):
    ...
    pricing_unit = models.CharField(
        ...
        default='PER_MILLION',
        help_text=_("Unit for pricing (OpenAI/Anthropic use million, AWS Bedrock uses thousand)")
    )
    ...
    context_window = models.IntegerField(
        null=True,
        blank=True,
        help_text=_("Maximum context window in tokens")
    )
...
class TaskOutputEstimate(models.Model):
    ...
    output_ratio = models.DecimalField(
        max_digits=5,
        decimal_places=4,
        help_text=_("Expected output tokens as ratio of input tokens (e.g., 0.15 for 15%)")
    )
...
class AIJobDefinition(models.Model):
    ...
    default_provider = models.CharField(
        max_length=50,
        help_text=_("Default provider (e.g., 'AWS_BEDROCK', 'OPENAI')")
    )
    default_model = models.CharField(
        max_length=100,
        help_text=_("Default model name")
    )
    ...
    system_prompt = models.TextField(
        null=True,
        blank=True,
        help_text=_("System/instruction prompt template")
    )
    ...
    fixed_output_tokens = models.IntegerField(
        null=True,
        blank=True,
        help_text=_("Fixed output token count")
    )
    ...
    items_per_batch_context = models.TextField(
        null=True,
        blank=True,
        help_text=_("Description of how batching works (e.g., '3 summaries at a time')")
    )
    ...
    max_concurrent_executions = models.IntegerField(
        default=1,
        help_text=_("Maximum concurrent executions allowed")
    )
    ...
    algorithm_module = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("Python module path for the job implementation (e.g., 'core.jobs.daily_summary')")
    )
...
class AIJobExecution(models.Model):
    ...
    items_processed = models.IntegerField(
        default=0,
        help_text=_("Number of items processed (e.g., documents, summaries)")
    )
    items_scope = models.JSONField(
        null=True,
        blank=True,
        help_text=_("IDs or identifiers of items in scope")
    )
    ...
    result_summary = models.JSONField(
        null=True,
        blank=True,
        help_text=_("Summary of results")
    )
    ...
    def cost_variance_percentage(self) -> Decimal:
        ...
        if self.estimated_cost_usd and self.actual_cost_usd and self.estimated_cost_usd > 0:
            variance = (self.actual_cost_usd - self.estimated_cost_usd) / self.estimated_cost_usd
...
class AIJobExecutionItem(models.Model):
    ...
    item_id = models.IntegerField(
        help_text=_("ID of the item being processed")
    )
