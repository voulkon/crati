...
from core.models.entities import AFMEntity, DecisionEntityRelationship, EntityRole, EntityType
...
class AFMExtractionService:
    ...
    def extract_afms_from_decision(self, decision: Decision, save_to_db: bool = True) -> List[Dict[str, Any]]:
        """
    ...
    def extract_afms_from_decisions_batch(self, decisions: List[Decision], save_to_db: bool = True) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'total_entities': sum(len(entities) for entities in batch_entities.values()),
            'stats': self.extraction_stats.copy()
        }

    def _find_afm_patterns_in_data(self, data: Any, parent_path: str = "") -> List[Dict]:
    ...
    def _extract_entities_from_pattern(self, pattern: Dict, decision: Decision) -> List[Dict[str, Any]]:
    ...
    def _detect_entity_type(self, data: Dict[str, Any]) -> str:
        ...
        # Company indicators
        company_terms = ['εταιρ', 'company', 'επων', 'business', 'corp', 'ltd', 'αε', 'οε', 'επε']
    ...
    def _calculate_confidence(self, pattern: Dict) -> float:
        ...
        if any(key for key in pattern['data'].keys() if 'name' in key.lower() or 'όνομα' in key.lower()):
            confidence += 0.1
        
        return min(confidence, 1.0)

    @transaction.atomic
    def _save_entities_to_database(self, decision: Decision, entities: List[Dict[str, Any]]):
        ...
        for entity_info in entities:
            ...
            # Create relationship
            relationship, rel_created = DecisionEntityRelationship.objects.get_or_create(
                decision=decision,
                entity=entity,
            ...
            if rel_created:
                ...
                logger.debug(f"Created relationship: {decision.ada} -> {entity.afm} ({entity_info['role']})")
...
def extract_afms_from_decision(decision: Decision, save_to_db: bool = True) -> List[Dict[str, Any]]:
...
def extract_afms_from_decisions(decisions: List[Decision], save_to_db: bool = True) -> Dict[str, Any]:
