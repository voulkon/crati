"""
Pre-calculation for direct-assignments/top-entities/ endpoint.

Two-layer design:
  compute_da_top_entities(…)    → pure DB query
  warm_da_top_entities_window(…) → calls compute + caches under exact Redis keys
"""
...
def warm_da_top_entities_window(
    ...
    _validate_dates(start_date_str, end_date_str, "warm_da_top_entities_window")
    ...
    for sort_by in ("amount", "frequency"):
        ...
        cache_paginated_offset(
            ...
            log_label=f"da_top_entities sort_by={sort_by}",
