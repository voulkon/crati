...
from core.models.entities import AFMEntity, DecisionEntityRelationship, DecisionAmountField
...
class DecisionHealthGuaranteeService:
    ...
    def _log_sample_adas(self, queryset, total_count: int, item_type: str = "decisions"):
        """
        ...
        if total_count > 3:
            last_items = list(queryset[max(0, total_count-3):].values_list('ada', flat=True))
        else:
            last_items = []
    ...
    def _log_sample_documents(self, queryset, total_count: int):
        ...
        # Get last 3 (if more than 3 total)
        if total_count > 3:
            last_items = list(queryset.select_related('decision')[max(0, total_count-3):])
        else:
            last_items = []
    ...
    def ensure_organization_resolution(
        ...
        """
        logger.info("🔍 Checking for decisions with unresolved organizations...")
        ...
        if total_missing == 0:
            logger.info("✅ All decisions have resolved organizations")
            return {
                'status': 'completed',
                'total_missing': 0,
                'resolved': 0,
        ...
        logger.warning(f"⚠️ Found {total_missing} decisions with unresolved organizations")
        self._log_sample_adas(decisions_needing_resolution, total_missing)  # Add this
        
        if dry_run:
            # Get detailed breakdown
            sample = list(decisions_needing_resolution.values(
        ...
        for batch_start in range(0, total_missing, batch_size):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(
                        self._resolve_organizations_safe,
                        decision
                    ): decision.ada
        ...
        logger.info(
            f"✅ Organization resolution guarantee completed: "
    ...
    def ensure_entity_extraction(
        ...
        Returns:
            Dictionary with processing results
        """
        logger.info("🔍 Checking for decisions missing entity extraction...")
        ...
        base_query = Decision.objects.annotate(
            ...
            Q(has_entities=False) | Q(has_amounts=False)
        )
        ...
        if total_missing == 0:
            logger.info("✅ All decisions have entity and amount extraction")
            return {
                'status': 'completed',
                'total_missing': 0,
                'processed': 0,
        ...
        logger.warning(f"⚠️ Found {total_missing} decisions without entity/amount extraction")
        self._log_sample_adas(decisions_without_entities, total_missing)  # Add this
        
        if dry_run:
            # Get detailed breakdown
            sample = list(decisions_without_entities.annotate(
                entity_count=Count('entity_relationships'),
                amount_count=Count('amount_fields')
            ).values(
                'ada', 'issue_date', 'entity_count', 'amount_count'
            )[:10])
        ...
        for batch_start in range(0, total_missing, batch_size):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(
                        self._extract_entities_and_amounts_safe, 
                        decision
                    ): decision.ada 
                    for decision in batch
                }
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result['success']:
                            results['processed'] += 1
                            logger.debug(
                                f"✓ {ada}: {result.get('entities_found', 0)} entities, "
        ...
        logger.info(
            f"✅ Entity extraction guarantee completed: "
    ...
    def ensure_company_enrichment(
        ...
        Returns:
            Dictionary with processing results
        """
        logger.info("🔍 Checking for entities missing company data...")
        
        # Find entities that haven't been attempted yet
        base_query = AFMEntity.objects.filter(
            gemi_lookup_attempted__isnull=True
        )
        ...
        if total_missing == 0:
            logger.info("✅ All entities have been attempted for company enrichment")
            return {
                'status': 'completed',
                'total_missing': 0,
                'queued': 0
        ...
        logger.warning(f"⚠️ Found {total_missing} entities needing company enrichment")
        self._log_sample_afms(entities_needing_lookup, total_missing)  # Add this
        ...
        logger.info(
            f"✅ Company enrichment guarantee completed: "
    ...
    def ensure_opensearch_indexing(
        ...
        Returns:
            Dictionary with processing results
        """
        logger.info("🔍 Checking for documents missing OpenSearch indexing...")
        
        # Find completed extractions with actual text content
        base_query = DocumentExtraction.objects.filter(
        ...
        if not settings.INDEX_THE_OPENSEARCH:
            logger.warning(
                f"⚠️ Found {total_extractions} completed document extractions, "
                f"but INDEX_THE_OPENSEARCH is disabled"
            )
            self._log_sample_documents(completed_extractions, total_extractions)  # Add this
        ...
        logger.info(f"Checking index status for {total_extractions} completed extractions...")
        ...
        if total_missing == 0:
            logger.info("✅ All documents are indexed in OpenSearch")
            return {
                'status': 'completed',
                'total_missing': 0,
                'indexed': 0,
        ...
        logger.warning(f"⚠️ Found {total_missing} documents missing from OpenSearch")
        ...
        logger.info(
            f"✅ OpenSearch indexing guarantee completed: "
    ...
    def ensure_all_decisions_health(
        ...
        """
        logger.info("=" * 80)
        logger.info("🏥 Starting comprehensive health guarantee check")
        logger.info("=" * 80)
        ...
        if decision_adas:
            logger.info(f"🎯 Targeting {len(decision_adas)} specific decisions")
        ...
        logger.info("\n" + "=" * 80)
        logger.info("📋 STEP 1/4: Organization Resolution")
        logger.info("=" * 80)
        results['steps']['organization_resolution'] = self.ensure_organization_resolution(
        ...
        logger.info("\n" + "=" * 80)
        logger.info("🏢 STEP 2/4: Entity & Amount Extraction")
        logger.info("=" * 80)
        ...
        logger.info("\n" + "=" * 80)
        logger.info("💼 STEP 3/4: Company Enrichment")
        logger.info("=" * 80)
        ...
        if decision_adas:
            ...
            logger.info(f"Targeting {len(afm_list)} unique AFMs from specified decisions")
        ...
        logger.info("\n" + "=" * 80)
        logger.info("🔍 STEP 4/4: OpenSearch Indexing")
        logger.info("=" * 80)
        ...
        # Generate summary
        logger.info("\n" + "=" * 80)
        logger.info("✅ Comprehensive health guarantee check completed")
        logger.info("=" * 80)
    ...
    def ensure_health_for_unhealthy_decisions(
        ...
        Returns:
            Dictionary with processing results
        """
        logger.info("🔍 Finding decisions with health issues...")
        ...
        if component_filter:
            problematic = self.health_service.get_problematic_decisions(
                ...
                status_filter=HealthStatus.ERROR
            )
        else:
            problematic = self.health_service.get_problematic_decisions(
                limit=1000
        ...
        if not problematic:
            logger.info("✅ No problematic decisions found")
            return {
                'status': 'completed',
                'total_found': 0,
                'fixed': 0
        ...
        logger.warning(f"⚠️ Found {len(problematic)} decisions with issues")
        ...
        for health_check in problematic:
            if health_check.organization_status in [HealthStatus.ERROR, HealthStatus.WARNING]:
                needs_organization.append(health_check.decision.ada)
            if health_check.entities_status in [HealthStatus.ERROR, HealthStatus.WARNING]:
                needs_entities.append(health_check.decision.ada)
            if health_check.opensearch_status in [HealthStatus.ERROR, HealthStatus.WARNING]:
        ...
        if needs_organization and (not component_filter or component_filter == 'organization'):
            logger.info(f"\n📋 Fixing {len(needs_organization)} decisions with organization issues")
            results['steps']['organization'] = self.ensure_organization_resolution(
                max_workers=max_workers,
                decision_adas=needs_organization
            )
        
        # Fix entities if needed
        if needs_entities and (not component_filter or component_filter == 'entities'):
            logger.info(f"\n🏢 Fixing {len(needs_entities)} decisions with entity issues")
            results['steps']['entities'] = self.ensure_entity_extraction(
                max_workers=max_workers,
                decision_adas=needs_entities
            )
            
            # Queue company enrichment for newly extracted entities
            if results['steps']['entities'].get('processed', 0) > 0:
                logger.info("\n💼 Queueing company enrichment for newly extracted entities")
                afm_list = list(
                    DecisionEntityRelationship.objects.filter(
        ...
        if needs_opensearch and (not component_filter or component_filter == 'opensearch'):
            logger.info(f"\n🔍 Fixing {len(needs_opensearch)} decisions with OpenSearch issues")
            results['steps']['opensearch'] = self.ensure_opensearch_indexing(
                max_workers=max_workers,
                decision_adas=needs_opensearch
            )
        
        # Re-check health to verify fixes
        logger.info("\n🏥 Re-checking health to verify fixes...")
        fixed_count = 0
        still_broken = []
        ...
        for health_check in problematic:
            ...
            if updated_health.overall_status == HealthStatus.HEALTHY:
                fixed_count += 1
            elif updated_health.overall_status in [HealthStatus.ERROR, HealthStatus.WARNING]:
                still_broken.append({
        ...
        logger.info(
            f"\n✅ Health-based guarantee completed: "
