...
class GreekGovernmentDocumentParser:
    ...
    def parse_document(self, text: str, ada: str = None) -> DocumentStructure:
        ...
        try:
            ...
            substance_start, matched_pattern = self._find_substance_start(normalized_text)
            ...
            footer_start, footer_pattern = self._find_footer_start(normalized_text, substance_start)
    ...
    def _calculate_confidence(self, result: DocumentStructure) -> float:
        ...
        if result.header and len(result.header) > 50:  # Header should have ministry info
            score += 0.25
        
        if result.substance and len(result.substance) > 100:  # Substance should be substantial
            score += 0.5
            
            # Check if substance contains decision-like content
            if re.search(r'εγκρίνουμε|εγκρίνω|αποφασίζουμε|αποφασίζω', result.substance, re.IGNORECASE):
                score += 0.15
    ...
    def _save_failed_sample(self, text: str, ada: str, result: DocumentStructure):
        ...
        filename = f"failed_{result.parse_status.value}_{ada or 'unknown'}_{timestamp}.json"
    ...
    def _analyze_document_sections(self, text: str) -> Dict:
        ...
        return {
            ...
            "lines_with_caps": [i for i, line in enumerate(lines) if line.isupper() and len(line) > 5],
            "potential_decisions": [i for i, line in enumerate(lines) if re.search(r'αποφασ|εγκριν|διαταγ', line, re.IGNORECASE)],
            "potential_signatures": [i for i, line in enumerate(lines) if re.search(r'υπουργ|γραμματε|διευθυντ', line, re.IGNORECASE)],
            "ada_mentions": [i for i, line in enumerate(lines) if re.search(r'αδα:', line, re.IGNORECASE)],
        }