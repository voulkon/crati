...
class EntityAmountExtractionService:
        ...
        try:
            with transaction.atomic():
                ...
                if save_to_db and entity_extractions:
                    entity_relationships = self._save_entities(decision, entity_extractions)
                ...
                if key in self.KNOWN_AFM_FIELDS:
                    ...
                    for afm_data in afms:
                        extractions.append({
                            ...
                            'raw_context': value if isinstance(value, dict) else {key: value}
                        })
                
                # Recurse into nested structures
                if isinstance(value, (dict, list)):
                    extractions.extend(self._extract_entities_known_fields(value, current_path))
                    
        elif isinstance(efv, list):
            for i, item in enumerate(efv):
                current_path = f"{parent_path}[{i}]"
                extractions.extend(self._extract_entities_known_fields(item, current_path))
    ...
    def _extract_entities_heuristic(
        ...
        if isinstance(efv, dict):
            for key, value in efv.items():
                ...
                # Recurse
                if isinstance(value, (dict, list)):
                    extractions.extend(self._extract_entities_heuristic(value, current_path))
    ...
    def _extract_afms_from_value(self, value: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(value, str) and self._is_valid_afm(value):
            ...
            name = value.get('name') or value.get('afmName') or value.get('sponsorAFMName')
    ...
    def _save_entities(
        ...
        for extraction in extractions:
            ...
            if not created:
                ...
                afm_entity.save(update_fields=['last_seen', 'total_appearances', 'name'])
