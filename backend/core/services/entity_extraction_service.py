...
class EntityExtractionService:
    ...
    def extract_afm_entities_from_decision(
        ...
        if skip_existing:
            ...
            if existing_count > 0:
                logger.debug(f"Decision {decision.ada} already processed, found {existing_count} relationships")
                return []
        ...
        logger.info(f"Extracted {len(afm_entities)} AFM entities from decision {decision.ada}")
        return afm_entities
    ...
    def _create_decision_entity_relationship(
        ...
        if created:
            logger.debug(f"Created relationship: {decision.ada} -> {entity.afm} ({role})")
    ...
    def get_entities_needing_company_data(self, limit: int = 100) -> List[AFMEntity]:
        ...
        entities = AFMEntity.objects.filter(
            ...
            Q(
                ...
                Q(
                    gemi_lookup_success=False,
                    gemi_lookup_attempted__lt=retry_cutoff
                )
            )
        ...
        for entity in entities:
            try:
                ...
                if companies:
                    ...
                    logger.success(f"Found {len(companies)} companies for AFM {entity.afm}")
                else:
                    stats['failed'] += 1
