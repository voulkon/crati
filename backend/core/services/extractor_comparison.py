...
from core.services.document_processor import BaseDocumentProcessor, DocumentAnalysisService
...
class ExtractorComparison(BaseDocumentProcessor):
    ...
    def compare_extractors(self, decision: Decision, include_async: bool = False) -> Dict[str, Any]:
        """
        ...
        logger.info(f"🔍 Comparing extractors for {decision.ada}")
        ...
        try:
            ...
            if self.sync_extractors:
                ...
                for extractor_name, extractor in self.sync_extractors.items():
                    logger.info(f"🔧 Running {extractor_name} on {decision.ada}")
                    ...
                    try:
                        ...
                        results["extractions"][extractor_name] = {
                            ...
                            "pages_data_count": len(result.pages_data) if result.pages_data else 0,
                            "extraction_type": "sync"
                        }
                        
                        logger.info(f"✅ {extractor_name}: {len(result.text)} chars, {result.page_count} pages, {processing_time}ms")
        ...
        results["comparison"] = self._generate_comparison_metrics(results["extractions"])
    ...
    def _process_async_extractors(self, decision: Decision, results: Dict[str, Any]):
        ...
        logger.info(f"🔄 Processing async extractors for {decision.ada}")
        
        analysis_service = DocumentAnalysisService()
        
        for provider in self.async_extractors:
            provider_name = provider.value
            logger.info(f"🔧 Running {provider_name} on {decision.ada} (async)")
            
            start_time = time.time()
            try:
                # Use the analysis service to process with the specific provider
                process_result = analysis_service.process_decision(decision, provider_name)
                processing_time = int((time.time() - start_time) * 1000)
                ...
                if process_result.get("success"):
                    ...
                    results["extractions"][provider_name] = {
                        "success": True,
                        "processing_time_ms": processing_time,
                        "text_length": len(extraction.raw_text) if extraction.raw_text else 0,
                    ...
                    logger.info(f"✅ {provider_name}: {len(extraction.raw_text or '')} chars, {extraction.page_count} pages, {processing_time}ms")
                else:
                    results["extractions"][provider_name] = {
                        "success": False,
                    ...
                    logger.error(f"❌ {provider_name} processing failed")
    ...
    def _generate_comparison_metrics(self, extractions: Dict[str, Any]) -> Dict[str, Any]:
        """Generate comparison metrics between extractors"""
        successful_extractions = {
            name: data for name, data in extractions.items() 
        ...
        text_lengths = {name: data["text_length"] for name, data in successful_extractions.items()}
        metrics["text_lengths"] = text_lengths
        
        if len(text_lengths) > 1:
            metrics["text_length_diff"] = max(text_lengths.values()) - min(text_lengths.values())
        
        # Page count comparison
        page_counts = {name: data["page_count"] for name, data in successful_extractions.items()}
        ...
        processing_times = {name: data["processing_time_ms"] for name, data in successful_extractions.items()}
        ...
        sync_extractors = [name for name, data in successful_extractions.items() 
                          if data.get("extraction_type") == "sync"]
        async_extractors = [name for name, data in successful_extractions.items() 
