...
class ImportJobQueue:
    ...
    def enqueue_job(
        ...
        logger.info(
            f"ImportJobQueue: Created ImportJob #{job.id} for {target_date}"
        )
    ...
    def dispatch_next_job(self) -> Optional[ImportJob]:
        ...
        next_job = ImportJob.objects.filter(
            status=ImportJobStatus.PENDING
        ...
        # ⚠️ CRITICAL: Update status to FETCHING BEFORE dispatching to prevent race conditions
    ...
    def on_job_completed(self, job_id: int):
        ...
        if dispatched:
            logger.info(
                f"ImportJobQueue: Auto-dispatched next job #{dispatched.id}"
            )
        else:
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        pending_jobs = ImportJob.objects.filter(
            status=ImportJobStatus.PENDING
        ).values('id', 'start_date', 'created_at')
    ...
    def clear_stale_jobs(self, max_age_hours: int = 24) -> int:
        ...
        if count > 0:
            ...
            logger.warning(
                f"ImportJobQueue: Marked {count} stale jobs as failed"
            )
