...
from core.models.organizations import Organization, Unit, SignerUnit, Signer
...
class OrganizationChartService:
    ...
    def get_organization_chart_data(self, org_uid: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            signers_with_units = SignerUnit.objects.filter(
                unit__organization=org
            ).values_list('signer_id', flat=True).distinct()
            
            direct_signers = Signer.objects.filter(
                organization=org
            ).exclude(pk__in=signers_with_units)
    ...
    def _build_unit_tree(self, unit: Unit) -> tuple[Dict[str, Any], List[Dict[str, Any]]]:
        """
        ...
        for signer_unit in SignerUnit.objects.filter(unit=unit).select_related('signer', 'position'):
