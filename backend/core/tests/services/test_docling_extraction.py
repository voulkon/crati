...
def test_docling_basic_extraction(docling_extractor, not_corrupted_file_path):
    ...
    """
    logger.info(f"🧪 Testing Docling extraction on: {not_corrupted_file_path}")
    
    try:
        result = docling_extractor.extract_text(str(not_corrupted_file_path))
        
        logger.success(f"✅ Extraction succeeded!")
        ...
        logger.error(f"❌ Extraction failed with: {type(e).__name__}")
        logger.error(f"   Error message: {str(e)}")
        logger.exception("Full traceback:")
        raise


def test_compare_pymupdf_vs_docling(pymupdf_extractor, docling_extractor, not_corrupted_file_path):
    """
    Compare PyMuPDF (known working) vs Docling side-by-side.
    """
    logger.info(f"\n{'='*60}")
    logger.info(f"📊 COMPARISON TEST: PyMuPDF vs Docling")
    logger.info(f"{'='*60}\n")
    ...
    try:
        ...
        logger.success(f"✅ PyMuPDF succeeded: {len(pymupdf_result.text)} chars")
    except Exception as e:
    ...
    try:
        ...
        logger.success(f"✅ Docling succeeded: {len(docling_result.text)} chars")
        
        # Compare results
        logger.info(f"\n📈 Comparison:")
        logger.info(f"  PyMuPDF chars: {len(pymupdf_result.text)}")
        logger.info(f"  Docling chars:  {len(docling_result.text)}")
        logger.info(f"  Difference: {abs(len(pymupdf_result.text) - len(docling_result.text))} chars")
        
        # Both should extract something
        assert docling_result.text, "Docling should extract text"
        assert len(docling_result.text) > 100, "Docling should extract meaningful content"
        
    except Exception as e:
        logger.error(f"❌ Docling failed: {type(e).__name__}: {str(e)}")
        logger.error(f"\n🔍 This is the REAL error we need to fix!")
        raise
...
def test_docling_all_test_files(docling_extractor, pdf_for_testing_path):
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            error = None
            logger.success(f"  ✅ Success - {char_count} chars")
        except Exception as e:
            success = False
            char_count = 0
    ...
    if success_count == 0:
        logger.error("\n⚠️  NONE of the PDFs could be processed!")
...
def test_docling_import_check():
    ...
    """
    logger.info("🔍 Checking Docling imports...")
    
    try:
        import docling
        logger.success(f"✅ docling package imported: {docling.__version__ if hasattr(docling, '__version__') else 'unknown version'}")
    except ImportError as e:
        logger.error(f"❌ Cannot import docling: {e}")
        pytest.fail("Docling package is not installed")
    
    try:
        from docling.document_converter import DocumentConverter
        logger.success("✅ DocumentConverter imported")
    except ImportError as e:
        logger.error(f"❌ Cannot import DocumentConverter: {e}")
        pytest.fail("DocumentConverter not available")
    
    try:
        from docling.chunking import HybridChunker
        logger.success("✅ HybridChunker imported")
    except ImportError as e:
        logger.error(f"❌ Cannot import HybridChunker: {e}")
        pytest.fail("HybridChunker not available")
    
    # Try to initialize converter
    try:
        converter = DocumentConverter()
        logger.success("✅ DocumentConverter initialized successfully")
    except Exception as e:
        logger.error(f"❌ Failed to initialize DocumentConverter: {type(e).__name__}: {e}")
        raise
    
    logger.success("\n✅ All Docling imports successful!")
