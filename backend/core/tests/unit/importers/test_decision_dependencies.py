...
def test_fetch_decision_with_missing_org_dependency(mock_diavgeia_fetcher):
    ...
    decision_dto = create_decision_dto(
        ...
        signer_ids=["100063912"],  # This signer references org 100054498 which doesn't exist yet
...
def test_org_cache_prevents_duplicate_fetches(mock_diavgeia_fetcher):
    ...
    assert Decision.objects.filter(ada__in=["CACHE_TEST_1", "CACHE_TEST_2"]).count() == 2
...
def test_real_api_call_for_inactive_org_dependency():
    ...
    real_ada = "Β1ΨΛ46ΜΤΛΠ-4ΒΩ"  # Replace with a real ADA that has the dependencies we want
    try:
