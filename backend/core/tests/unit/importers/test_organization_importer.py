...
def expected_defaults_mapping():
    to_return = {
        'abbreviation': None, 'category': 'NPDD', 'fek_issue': None, 
        'fek_number': None, 'fek_year': None, 'label': 'Foo SA',
        'latin_name': 'Foo', 
        'status': OrganizationStatus.ACTIVE.value , 
        'supervisor_org_name': None, 'supervisor_org_uid': None, 
        'vat_number': 'EL123', 
        'website': None
        }
    return to_return
...
@pytest.mark.fast
def test_to_defaults_mapping(
    organization_dto,
    expected_defaults_mapping
    ):
