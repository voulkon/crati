...
Covers the base-URL precedence order and the decision page URL builder:
    ...
    1. FRONTEND_DOMAINS_clean[0]  (full origin with scheme)
    2. FRONTEND_HOSTNAMES[0]      (hostname only -> https:// prefixed)
    3. DEBUG on                   -> http://localhost:3000
    4. fallback                   -> https://crati.co
"""
...
class TestFrontendBaseUrl:
    ...
    def test_uses_frontend_domains_clean_first(self):
        with override_settings(
            FRONTEND_DOMAINS_clean=["https://app.crati.co"],
