...
def analyze_discovery_overlap():
    ...
    first_source_counts = Decision.objects.values(
    ...
    only_org_specific = Decision.objects.filter(
    ...
    return {
        ...
        "multi_source_percentage": (multi_source.count() / Decision.objects.count() * 100) 
            if Decision.objects.count() > 0 else 0,
