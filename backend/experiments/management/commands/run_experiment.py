...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--decision-type', type=str, help='Filter by decision type UID')
        ...
        parser.add_argument('--compare-all', action='store_true', help='Run all strategies and compare')
    ...
    def handle(self, *args, **options):
        ...
        if options['compare_all']:
            ...
            for name, strategy_class in strategies_map.items():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"  ✓ {run.successful_count}/{run.total_decisions} "
            ...
            if run.failed_count > 0:
                ...
                for result in failures:
                    self.stdout.write(f"  {result.decision.ada}: {result.error_message[:100]}")
