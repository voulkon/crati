...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--decision-type', type=str, help='Filter to specific decision type folder (e.g., "Β.2.1_ΕΓΚΡΙΣΗ ΔΑΠΑΝΗΣ")')
        ...
        parser.add_argument('--export-results', action='store_true', help='Export extraction results to JSON files for review')
        parser.add_argument('--export-dir', type=str, default='extraction_results', help='Directory for exported results')
    ...
    def handle(self, *args, **options):
        ...
        tester = SampleTester(samples_dir, decision_type_filter=options.get('decision_type'))
    ...
    def _export_results(self, test_result: dict, strategy_name: str, export_dir: str, samples_dir: str):
        ...
        for sample_result in test_result['results']:
            ...
            result_entry = {
                ...
                'original_metadata': {
                    ...
                    'decision_type': original_data.get('decision_type', {}).get('label'),
                }
            }
                ...
                with open(detail_file, 'w', encoding='utf-8') as f:
                    json.dump({
                        ...
                        'comparison': {
                            ...
                            'has_beneficiary': bool(sample_result.data.get('beneficiary_name')),
                            'extracted_amount': sample_result.data.get('total_amount'),
                            'api_amount': original_data.get('amounts', {}).get('calculated_total'),
                            'amounts_match': self._amounts_match(
                                sample_result.data.get('total_amount'),
                                original_data.get('amounts', {}).get('calculated_total')
                            ),
                        }
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  Successful: {test_result['successful']} ({test_result['success_rate']:.1f}%)\n")
            ...
            for result in summary['results']:
                if result['success']:
                    ...
                    f.write(f"Original Subject: {result['original_metadata']['subject']}\n")
                    
                    extracted = result['extracted']
                    if extracted:
                        f.write(f"Extracted Purpose: {extracted.get('purpose', 'N/A')}\n")
                        f.write(f"Extracted Beneficiary: {extracted.get('beneficiary_name', 'N/A')}\n")
                        f.write(f"Extracted Amount: {extracted.get('total_amount', 'N/A')}\n")
                        f.write(f"API Amount: {result['original_metadata']['amount']}\n")
                        f.write(f"Confidence: {extracted.get('confidence_score', 'N/A')}\n")
                    f.write(f"\n{'-'*80}\n\n")
    ...
    def _amounts_match(self, extracted: float, api: float, tolerance: float = 0.01) -> bool:
