...
class ExperimentRun(models.Model):
    ...
    strategy_version = models.CharField(max_length=50, null=True, blank=True, help_text="Version/commit hash of the strategy")
    
    # Dataset definition
    decision_type = models.ForeignKey(ActType, on_delete=models.SET_NULL, null=True, blank=True)
    dataset_filter = models.JSONField(help_text="JSON representation of the queryset filters used")
    total_decisions = models.IntegerField()
    
    # Results summary
    successful_count = models.IntegerField(default=0)
    failed_count = models.IntegerField(default=0)
    success_rate = models.FloatField(null=True, blank=True, help_text="Percentage 0-100")
...
class ExperimentResult(models.Model):
    ...
    """
    run = models.ForeignKey(ExperimentRun, on_delete=models.CASCADE, related_name='results')
    ...
    extracted_data = models.JSONField(default=dict, help_text="Successfully extracted structured data")
    
    # Performance
    processing_time_ms = models.IntegerField(null=True, blank=True)
    
    # Quality metrics (optional, strategy-specific)
    confidence_score = models.FloatField(null=True, blank=True, help_text="0-1 confidence in the extraction")
    validation_errors = models.JSONField(default=list, help_text="List of validation issues found")
...
class StrategyConfiguration(models.Model):
    ...
    """
    decision_type = models.ForeignKey(ActType, on_delete=models.CASCADE, related_name='strategy_configs')
    strategy_name = models.CharField(max_length=255, db_index=True, help_text="Reference to the Strategy class")
    name = models.CharField(max_length=255, help_text="Human-readable name for this configuration")
    ...
    created_by = models.CharField(max_length=255, blank=True, help_text="Who created/approved this")
