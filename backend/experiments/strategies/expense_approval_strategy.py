...
class ExpenseApprovalData(BaseModel):
    ...
    purpose: Optional[str] = Field(None, description="Payment purpose/reason - THE GOLD!")
    ...
    # References
    references: List[str] = Field(default_factory=list, description="ΑΑΥ, Αρ.Παρ., etc.")
    
    # Funding info
    funding_source: Optional[str] = Field(None, description="Funding source")
    account_type: Optional[str] = Field(None, description="Account type (ΤΑΚΤΙΚΑ, etc.)")
...
class ExpenseApprovalStrategy(DecompositionStrategy):
    ...
    def decompose(self, decision: Decision, text: str) -> DecompositionResult:
        ...
        try:
            ...
            if self.debug_mode:
                ...
                logger.info(f"  Purpose: {data.purpose[:50] if data.purpose else 'NONE'}")
    ...
    def _extract_single(self, text: str, pattern_key: str, ada: str = 'UNKNOWN') -> Optional[str]:
        """Extract a single value using a regex pattern."""
        match = self.compiled_patterns[pattern_key].search(text)
        if match:
            value = match.group(1).strip()
            if self.debug_mode:
                logger.debug(f"[{ada}] {pattern_key}: {value[:50] if value else 'EMPTY'}")
    ...
    def _extract_amount(self, text: str, pattern_key: str, ada: str = 'UNKNOWN') -> Optional[float]:
        ...
        if amount_str:
            ...
            try:
                ...
                if self.debug_mode:
                    logger.warning(f"[{ada}] Failed to parse {pattern_key}: {amount_str}")
        return None
    ...
    def _extract_purpose(self, text: str, ada: str = 'UNKNOWN') -> Optional[str]:
        ...
        budget_codes = self._extract_budget_codes(text, ada)
        if budget_codes:
            descriptions = [bc.description for bc in budget_codes 
                          if bc.description and len(bc.description) > 20]
            if descriptions:
                if self.debug_mode:
                    logger.debug(f"[{ada}] Purpose (budget desc): {descriptions[0][:80]}")
    ...
    def _extract_beneficiary_address(self, text: str, ada: str = 'UNKNOWN') -> Optional[str]:
    ...
    def _extract_budget_codes(self, text: str, ada: str = 'UNKNOWN') -> List[BudgetCode]:
