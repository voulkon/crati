...
class KeySectionsStrategy(DecompositionStrategy):
    ...
    def decompose(self, decision: Decision, text: str) -> DecompositionResult:
        ...
        desc_match = re.search(r'Περιγραφή λογ/μου[\s\n]+Ποσό[\s\n]+[\d.]+[\s\n]+([^\d]+?)\s+[\d,]+', text, re.DOTALL)
        if desc_match:
        ...
        reason_match = re.search(r'ΑΙΤΙΑ ΠΛΗΡΩΜΗΣ[\s\n]+(.+?)(?:Κράτηση|ΣΥΝΟΔΕΥΤΙΚΑ)', text, re.DOTALL)
        if reason_match:
...
class ContentFilterStrategy(DecompositionStrategy):
    ...
    def decompose(self, decision: Decision, text: str) -> DecompositionResult:
        ...
        combined_searchable = ' '.join(s['content'] for s in data['searchable_sections'])
        ...
        if data['searchable_sections']:
            ...
            return DecompositionResult(
                success=False, 
                error="No searchable content sections identified"
            )
