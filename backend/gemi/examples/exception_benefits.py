...
def bad_api_call():
    try:
        response = requests.get("https://api.example.com/companies/123")
...
class CompanyNotFoundError(Exception):
    def __init__(self, company_id: str, api_message: str = None):
        ...
        super().__init__(f"Company {company_id} not found: {api_message or 'Unknown error'}")

def good_api_call(company_id: str):
    try:
        response = requests.get(f"https://api.example.com/companies/{company_id}")
