...
class CompaniesService(CachedClient):
    ...
    def __init__(self, base_client: BaseAPIClient, cache=None):
        super().__init__(cache or MemoryCache(default_ttl=300))  # 5 min cache for company data
    ...
    def search_companies(
        ...
        if isinstance(data, dict) and "searchResults" in data:
            ...
            items = [CompanySummary.model_validate(item) for item in data["searchResults"]]
            ...
            return PaginatedResponse[CompanySummary](
                ...
                page=page,
                page_size=page_size,
                has_next=(metadata.get("resultsOffset", 0) + len(items)) < metadata.get("totalCount", 0),
