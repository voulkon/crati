...
class TestNotificationBatchListAPI:
    ...
    def test_filter_batches_by_subscription(
        ...
        from conftest import NotificationSubscriptionFactory, NotificationBatchFactory, OrganizationFactory
    ...
    def test_user_only_sees_own_batches(
        self, authenticated_client, user, api_client
    ):
        """Test that users can only see their own batches"""
        from conftest import UserFactory, NotificationBatchFactory, NotificationSubscriptionFactory, OrganizationFactory
        
        # Create another user with a batch
        other_user = UserFactory()
        other_org = OrganizationFactory()
        other_sub = NotificationSubscriptionFactory(user=other_user, organization=other_org)
...
class TestNotificationBatchDetailAPI:
    ...
    def test_cannot_access_other_users_batch(
        self, authenticated_client, user
    ):
        """Test that users cannot access other users' batches"""
        from conftest import (
            UserFactory, NotificationBatchFactory, 
            NotificationSubscriptionFactory, OrganizationFactory
        )
        ...
        # Try to access it
        response = authenticated_client.get(f'/api/notifications/batches/{other_batch.id}/')
...
class TestNotificationBatchDecisionsAPI:
    ...
    def test_get_batch_decisions(
        ...
        from conftest import (
            NotificationBatchFactory, NotificationBatchDecisionFactory,
            DecisionFactory
        )
    ...
    def test_batch_decisions_pagination(
        ...
        # Create 25 decisions
        for i in range(25):
            decision = DecisionFactory(organization=notification_subscription.organization)
    ...
    def test_filter_batch_decisions_by_viewed(
        ...
        viewed_decision = DecisionFactory(organization=notification_subscription.organization)
        ...
        unviewed_decision = DecisionFactory(organization=notification_subscription.organization)
        unviewed_bd = NotificationBatchDecisionFactory(
            batch=batch,
            decision=unviewed_decision,
            is_viewed=False
        )
