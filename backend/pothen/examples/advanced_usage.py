...
from pothen.config import PothenConfig, ScrapingConfig, LoggingConfig
...
def main():
    ...
    config = PothenConfig(
        ...
        logging=LoggingConfig(
            level="DEBUG",
            file_path=Path("./logs/pothen_advanced.log")
        )
    )
    ...
    with PothenClient(
        ...
        try:
            ...
            for year, result in results.items():
                ...
                mp_names = [content.mp_name for content in result['extracted_content'] 
                          if content.mp_name]
                if mp_names:
        ...
        for year, year_stats in stats['by_year'].items():
            print(f"    {year}: {year_stats['files']} files "
...
def demonstrate_error_handling():
    ...
    with PothenClient() as client:
        ...
        try:
            ...
            print(f"Unexpectedly found declarations for 1999: {len(declarations.entries)}")
        except Exception as e:
        ...
        try:
            ...
            if years:
                ...
                print(f"Success with limit: {len(result['pdf_metadata'])} PDFs downloaded")
        except Exception as e:
