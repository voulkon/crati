...
class ScrapingConfig(BaseModel):
    ...
    user_agent: str = Field(default="pothen-scraper/1.0.0", description="User agent string")
    
    # Download settings
    download_dir: Path = Field(default=Path("./pothen_downloads"), description="Directory for downloads")
    ...
    preferred_pdf_method: str = Field(default="pdfplumber", description="Preferred PDF extraction method")
    min_content_length: int = Field(default=100, description="Minimum content length for valid extraction")
...
class PothenConfig(BaseModel):
    ...
    def from_env(cls) -> "PothenConfig":
        ...
        if os.getenv("POTHEN_MAX_RETRIES"):
            config_dict["scraping"]["max_retries"] = int(os.getenv("POTHEN_MAX_RETRIES"))
        
        if os.getenv("POTHEN_RATE_LIMIT"):
            config_dict["scraping"]["requests_per_minute"] = int(os.getenv("POTHEN_RATE_LIMIT"))
        
        if os.getenv("POTHEN_DOWNLOAD_DIR"):
            config_dict["scraping"]["download_dir"] = Path(os.getenv("POTHEN_DOWNLOAD_DIR"))
        
        if os.getenv("POTHEN_USER_AGENT"):
            config_dict["scraping"]["user_agent"] = os.getenv("POTHEN_USER_AGENT")
        
        if os.getenv("POTHEN_PDF_METHOD"):
            config_dict["scraping"]["preferred_pdf_method"] = os.getenv("POTHEN_PDF_METHOD")
