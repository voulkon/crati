...
from .constants import PDF_CHUNK_SIZE, MAX_PDF_SIZE_MB
...
class PDFDownloader:
    ...
    def generate_filename(self, declaration: DeclarationEntry) -> str:
        ...
        if original_filename.endswith('.pdf'):
            ...
            safe_name = f"{declaration.last_name}_{declaration.first_name}_{declaration.year}"
    ...
    def download_declaration(
        ...
        try:
            ...
            logger.info(f"Successfully downloaded PDF: {downloaded_path} ({metadata.filesize_mb:.2f} MB)")
            return metadata
            
        except Exception as e:
            raise PothenDownloadError(f"Failed to download {declaration.pdf_url}: {str(e)}") from e
    ...
    def download_declarations(
        ...
        for i, declaration in enumerate(declarations):
            ...
            try:
                logger.info(f"Processing declaration {i+1}/{len(declarations)}: {declaration.full_name}")
                ...
                logger.error(f"Failed to download declaration for {declaration.full_name}: {str(e)}")
                continue
        
        logger.info(f"Downloaded {downloaded_count} PDFs out of {len(declarations)} declarations")
        return results
    ...
    def _validate_pdf_file(self, filepath: Path) -> None:
        ...
        try:
            ...
            raise PothenValidationError(f"Cannot read downloaded file: {filepath}") from e
