...
class PDFTextExtractor:
    ...
    def __init__(self, preferred_method: str = "pdfplumber"):
        ...
        if not available_methods:
            raise RuntimeError("No PDF processing libraries available. Install pdfplumber or PyPDF2.")
        
        if preferred_method not in available_methods:
            self.preferred_method = available_methods[0]
            logger.warning(f"Preferred method '{preferred_method}' not available. Using '{self.preferred_method}'")
    ...
    def extract_text_pypdf2(self, filepath: Path) -> tuple[str, int]:
        ...
        try:
            ...
            raise PothenParsingError(f"PyPDF2 extraction failed for {filepath}: {str(e)}") from e
    ...
    def extract_text_pdfplumber(self, filepath: Path) -> tuple[str, int]:
        ...
        try:
            ...
            raise PothenParsingError(f"pdfplumber extraction failed for {filepath}: {str(e)}") from e
    ...
    def extract_text(self, filepath: Path) -> ParsedDeclarationContent:
        ...
        try:
            ...
            try:
                ...
                raise PothenParsingError(f"All extraction methods failed: {str(fallback_e)}") from fallback_e
        
        # Validate extracted text
        if len(text.strip()) < MIN_PDF_CONTENT_LENGTH:
            raise PothenValidationError(f"Extracted text too short ({len(text)} chars) from {filepath}")
    ...
    def extract_from_metadata_list(self, pdf_metadata_list: List[PDFMetadata]) -> List[ParsedDeclarationContent]:
        """Extract text from multiple PDFs."""
        results = []
        
        for i, metadata in enumerate(pdf_metadata_list):
            try:
                logger.info(f"Processing PDF {i+1}/{len(pdf_metadata_list)}: {metadata.filepath.name}")
                
                parsed_content = self.extract_text(metadata.filepath)
                results.append(parsed_content)
                
            except Exception as e:
                logger.error(f"Failed to extract text from {metadata.filepath}: {str(e)}")
                continue
        
        logger.info(f"Successfully extracted text from {len(results)} out of {len(pdf_metadata_list)} PDFs")
        return results